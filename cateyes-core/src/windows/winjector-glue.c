...
gboolean
cateyes_winjector_helper_instance_is_process_still_running (void * handle)
{
  DWORD exit_code;
...
void
cateyes_winjector_helper_instance_close_process_handle (void * handle)
{
...
void *
cateyes_winjector_helper_factory_spawn (const gchar * path, const gchar * parameters, CateyesWinjectorPrivilegeLevel level, GError ** error)
{
  ...
  if (level == CATEYES_WINJECTOR_PRIVILEGE_LEVEL_ELEVATED)
    ei.lpVerb = L"runas";
  else
    ei.lpVerb = L"open";
...
void
cateyes_winjector_resource_store_set_acls_as_needed (const gchar * path, GError ** error)
{
  ...
  path_utf16 = (WCHAR *) g_utf8_to_utf16 (path, -1, NULL, NULL, NULL);
  sddl = cateyes_access_get_sddl_string_for_temp_directory ();
