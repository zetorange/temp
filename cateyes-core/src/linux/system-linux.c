...
cateyes_system_enumerate_applications (int * result_length)
{
  ...
  applications = g_array_new (FALSE, TRUE, sizeof (CateyesHostApplicationInfo));
  ...
  while ((proc_name = g_dir_read_name (proc_dir)) != NULL)
  {
    ...
    gchar * app_id;
    gchar * app_name;
    CateyesHostApplicationInfo * info;
    ...
    info = &g_array_index (applications, CateyesHostApplicationInfo, applications->len - 1);
  ...
  *result_length = applications->len;
  ...
  return (CateyesHostApplicationInfo *) g_array_free (applications, FALSE);
}
...
cateyes_system_enumerate_processes (int * result_length)
{
  ...
  while ((proc_name = g_dir_read_name (proc_dir)) != NULL)
  {
    ...
    info = &g_array_index (processes, CateyesHostProcessInfo, processes->len - 1);
    cateyes_host_process_info_init (info, pid, name, &no_icon, &no_icon);
