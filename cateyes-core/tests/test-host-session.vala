namespace Cateyes.HostSessionTest {
	...
	private static async void resource_leaks (Harness h) {
		try {
			var device_manager = new DeviceManager ();
			var device = yield device_manager.get_device_by_type (DeviceType.LOCAL);
			var process = Cateyes.Test.Process.start (Cateyes.Test.Labrats.path_to_executable ("sleeper"));
	...
	namespace Linux {
		...
		private static async void spawn (Harness h) {
			if ((Cateyes.Test.os () == Cateyes.Test.OS.ANDROID || Cateyes.Test.os_arch_suffix () == "-linux-arm") && !GLib.Test.slow ()) {
				stdout.printf ("<skipping, run in slow mode> ");
				h.done ();
				return;
			}
			...
			try {
				...
				options.stdio = PIPE;
				pid = yield host_session.spawn (Cateyes.Test.Labrats.path_to_executable ("sleeper"), options);
		...
		private static async void fork (Harness h) {
			yield Unix.run_fork_scenario (h, Cateyes.Test.Labrats.path_to_executable ("forker"));
		}

		private static async void fork_plus_exec (Harness h, string method) {
			yield Unix.run_fork_plus_exec_scenario (h, Cateyes.Test.Labrats.path_to_executable ("spawner"), method);
		}

		private static async void bad_exec (Harness h) {
			yield Unix.run_bad_exec_scenario (h, Cateyes.Test.Labrats.path_to_executable ("spawner"), "execv");
		}
...
""");
	...
	namespace Darwin {
		...
		private static async void backend (Harness h) {
			...
			if (Cateyes.Test.os () == Cateyes.Test.OS.MACOS) {
				var icon = prov.icon;
				assert (icon != null);
		...
		private static async void spawn_native (Harness h) {
			var target_name = (Cateyes.Test.os () == Cateyes.Test.OS.MACOS) ? "sleeper-macos" : "sleeper-ios";
			yield run_spawn_scenario (h, target_name);
		}

		private static async void spawn_other (Harness h) {
			var target_name = (Cateyes.Test.os () == Cateyes.Test.OS.MACOS) ? "sleeper-macos32" : "sleeper-ios32";
			yield run_spawn_scenario (h, target_name);
		}
		...
		private static async void run_spawn_scenario (Harness h, string target_name) {
			...
			try {
				...
				var options = HostSpawnOptions ();
				options.stdio = PIPE;
				pid = yield host_session.spawn (Cateyes.Test.Labrats.path_to_file (target_name), options);
		...
		private static async void spawn_without_attach_native (Harness h) {
			var target_name = (Cateyes.Test.os () == Cateyes.Test.OS.MACOS) ? "stdio-writer-macos" : "stdio-writer-ios";
			yield run_spawn_scenario_with_stdio (h, target_name);
		}

		private static async void spawn_without_attach_other (Harness h) {
			var target_name = (Cateyes.Test.os () == Cateyes.Test.OS.MACOS) ? "stdio-writer-macos32" : "stdio-writer-ios32";
			yield run_spawn_scenario_with_stdio (h, target_name);
		}
...
""");
		...
		private static async void fork_native (Harness h) {
			var target_name = (Cateyes.Test.os () == Cateyes.Test.OS.MACOS) ? "forker-macos" : "forker-ios";
			yield Unix.run_fork_scenario (h, Cateyes.Test.Labrats.path_to_file (target_name));
		}

		private static async void fork_other (Harness h) {
			var target_name = (Cateyes.Test.os () == Cateyes.Test.OS.MACOS) ? "forker-macos32" : "forker-ios32";
			yield Unix.run_fork_scenario (h, Cateyes.Test.Labrats.path_to_file (target_name));
		}
		...
		private static async void posix_spawn (Harness h) {
			yield Unix.run_posix_spawn_scenario (h, Cateyes.Test.Labrats.path_to_executable ("spawner"));
		}

		private static async void posix_spawn_plus_setexec (Harness h) {
			yield Unix.run_posix_spawn_plus_setexec_scenario (h, Cateyes.Test.Labrats.path_to_executable ("spawner"));
		}
		...
		namespace Manual {
			...
			private static async void spawn_ios_app (Harness h) {
				...
				try {
					...
					string? url = "https://www.cateyes.re/docs/ios/";
					*/
...
""");
				...
				assert (parent_detach_reason == "CATEYES_SESSION_DETACH_REASON_PROCESS_TERMINATED");
				...
				assert (child_detach_reason == "CATEYES_SESSION_DETACH_REASON_PROCESS_TERMINATED");
		...
		public static async void run_fork_plus_exec_scenario (Harness h, string target_path, string method) {
			try {
				...
				assert (child_pre_exec_detach_reason == "CATEYES_SESSION_DETACH_REASON_PROCESS_REPLACED");
...
""");
				...
				assert (child_post_exec_detach_reason == "CATEYES_SESSION_DETACH_REASON_PROCESS_TERMINATED");
		...
		public static async void run_bad_exec_scenario (Harness h, string target_path, string method) {
			try {
				...
				assert (detach_reason == "CATEYES_SESSION_DETACH_REASON_PROCESS_TERMINATED");
...
""");
		...
		public static async void run_posix_spawn_plus_setexec_scenario (Harness h, string target_path) {
			...
			try {
				...
				assert (pre_exec_detach_reason == "CATEYES_SESSION_DETACH_REASON_PROCESS_REPLACED");
...
""");
				...
				assert (post_exec_detach_reason == "CATEYES_SESSION_DETACH_REASON_PROCESS_TERMINATED");
	...
	namespace Windows {
		...
		private static async void create_process (Harness h) {
			...
			var target_path = Cateyes.Test.Labrats.path_to_executable ("spawner");
...
""");
	...
	namespace Fruity {
		...
		private static async void large_messages (Harness h) {
			...
			try {
				stdout.printf ("connecting to cateyes-server\n");
				var host_session = yield prov.create ();
		...
		namespace PropertyList {
			...
			private static void can_construct_from_xml_document () {
				var xml =
					...
					"</plist>\n";
				try {
					var plist = new Cateyes.Fruity.PropertyList.from_xml (xml);
			...
			private static void to_xml_yields_complete_document () {
				var plist = new Cateyes.Fruity.PropertyList ();
				plist.set_string ("MessageType", "Detached");
				plist.set_int ("DeviceID", 2);

				var proplist = new Cateyes.Fruity.PropertyList ();
	...
	public class Harness : Cateyes.Test.AsyncHarness {
		...
		public Harness (owned Cateyes.Test.AsyncHarness.TestSequenceFunc func) {
			base ((owned) func);
		}

		public Harness.without_timeout (owned Cateyes.Test.AsyncHarness.TestSequenceFunc func) {
			base ((owned) func);
			timeout = 0;
		}
