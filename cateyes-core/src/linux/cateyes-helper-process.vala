...
#if ANDROID
			SELinux.setfilecon (tempdir.path, "u:object_r:cateyes_file:s0");
#endif

			var blob32 = Cateyes.Data.Helper.get_cateyes_helper_32_blob ();
			if (blob32.data.length > 0) {
				helper32 = new TemporaryFile.from_stream ("cateyes-helper-32",
					new MemoryInputStream.from_data (blob32.data, null),
					tempdir);
				FileUtils.chmod (helper32.path, 0700);
			}

			var blob64 = Cateyes.Data.Helper.get_cateyes_helper_64_blob ();
			if (blob64.data.length > 0) {
				helper64 = new TemporaryFile.from_stream ("cateyes-helper-64",
