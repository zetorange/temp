...
static void cateyes_child_process_on_death (GPid pid, gint status, gpointer user_data);

static WCHAR * cateyes_argv_to_command_line (const gchar ** argv, gint argv_length);
static WCHAR * cateyes_envp_to_environment_block (const gchar ** envp, gint envp_length);

static void cateyes_append_n_backslashes (GString * str, guint n);

static void cateyes_make_pipe (HANDLE * read, HANDLE * write);
static void cateyes_ensure_not_inherited (HANDLE handle);

CateyesImageData *
_cateyes_windows_host_session_provider_try_extract_icon (void)
{
  CateyesImageData * result = NULL;
  ...
  while (result == NULL && IEnumIDList_Next (children, 1, &child, NULL) == S_OK)
  {
    ...
    result = _cateyes_image_data_from_native_icon_handle (file_info.hIcon, CATEYES_ICON_SMALL);
...
CateyesChildProcess *
_cateyes_windows_host_session_spawn (CateyesWindowsHostSession * self, const gchar * path, CateyesHostSpawnOptions * options, GError ** error)
{
  CateyesChildProcess * process = NULL;
  ...
  CateyesStdioPipes * pipes;
  guint watch_id;
  GSource * watch;

  application_name = (WCHAR *) g_utf8_to_utf16 (path, -1, NULL, NULL, NULL);

  if (cateyes_host_spawn_options_get_has_argv (options))
  {
    gchar ** argv;
    gint argv_length;

    argv = cateyes_host_spawn_options_get_argv (options, &argv_length);
    command_line = cateyes_argv_to_command_line (argv, argv_length);
  }
  else
  {
    command_line = NULL;
  ...
  if (cateyes_host_spawn_options_get_has_envp (options) || cateyes_host_spawn_options_get_has_env (options))
  {
    gchar ** envp;
    gint envp_length;

    envp = cateyes_host_spawn_options_compute_envp (options, &envp_length);
    environment = cateyes_envp_to_environment_block (envp, envp_length);
    g_strfreev (envp);
  }
  else
  {
  ...
  switch (stdio)
  {
    ...
    case CATEYES_STDIO_PIPE:
      cateyes_make_pipe (&stdin_read, &stdin_write);
      cateyes_make_pipe (&stdout_read, &stdout_write);
      cateyes_make_pipe (&stderr_read, &stderr_write);

      cateyes_ensure_not_inherited (stdin_write);
      cateyes_ensure_not_inherited (stdout_read);
      cateyes_ensure_not_inherited (stderr_read);
  ...
  process = cateyes_child_process_new (
      G_OBJECT (self),
...
gboolean
_cateyes_windows_host_session_process_is_alive (guint pid)
{
  HANDLE process;
  DWORD res;
...
void
cateyes_child_process_close (CateyesChildProcess * self)
{
  GSource * watch;
  ...
  watch = cateyes_child_process_get_watch (self);
  if (watch != NULL)
    g_source_destroy (watch);

  CloseHandle (cateyes_child_process_get_handle (self));
  CloseHandle (cateyes_child_process_get_main_thread (self));

  self->closed = TRUE;
}

void
cateyes_child_process_resume (CateyesChildProcess * self, GError ** error)
{
  if (self->resumed)
    goto already_resumed;

  ResumeThread (cateyes_child_process_get_main_thread (self));
...
static void
cateyes_child_process_on_death (GPid pid, gint status, gpointer user_data)
{
  CateyesChildProcess * self = user_data;

  (void) pid;

  _cateyes_windows_host_session_on_child_dead (
      CATEYES_WINDOWS_HOST_SESSION (cateyes_child_process_get_parent (self)),
      self,
      status);
}

static WCHAR *
cateyes_argv_to_command_line (const gchar ** argv, gint argv_length)
{
  GString * line;
  WCHAR * line_utf16;
  gint i;
...
static WCHAR *
cateyes_envp_to_environment_block (const gchar ** envp, gint envp_length)
{
  GString * block;
...
static void
cateyes_make_pipe (HANDLE * read, HANDLE * write)
{
...
static void
cateyes_ensure_not_inherited (HANDLE handle)
{
