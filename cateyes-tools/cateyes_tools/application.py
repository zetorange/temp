...
class ConsoleApplication(object):
    def __init__(self, run_until_return=await_enter, on_stop=None):
        ...
        parser = OptionParser(usage=self._usage(), version=cateyes.__version__)
        ...
        if self._needs_device():
            ...
            parser.add_option("-R", "--remote", help="connect to remote cateyes-server",
                    action='store_const', const='remote', dest="device_type", default=None)
            parser.add_option("-H", "--host", help="connect to remote cateyes-server on HOST",
    ...
    def run(self):
        mgr = cateyes.get_device_manager()
    ...
    def _try_start(self):
        ...
        if self._device_id is not None:
            try:
                self._device = cateyes.get_device(self._device_id)
            except:
            ...
            self._device = cateyes.get_device_manager().add_remote_device(self._host)
        else:
            self._device = cateyes.get_local_device()
...
def find_device(type):
    for device in cateyes.enumerate_devices():
        if device.type == type:
            return device
    return None
