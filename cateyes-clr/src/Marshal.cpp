...
namespace Cateyes
{
  ...
  ImageSource ^
  Marshal::CateyesIconToImageSource (CateyesIcon * icon)
  {
    if (icon == NULL)
      return nullptr;

    gint width = cateyes_icon_get_width (icon);
    gint height = cateyes_icon_get_height (icon);
    gint rowstride = cateyes_icon_get_rowstride (icon);
    gsize pixelsSize;
    guint8 * pixelsRgba = static_cast<guint8 *> (const_cast<gpointer> (g_bytes_get_data (cateyes_icon_get_pixels (icon), &pixelsSize)));
