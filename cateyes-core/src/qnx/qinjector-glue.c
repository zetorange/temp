...
#define CATEYES_REMOTE_PAYLOAD_SIZE (8192)
#define CATEYES_REMOTE_DATA_OFFSET (512)
#define CATEYES_REMOTE_STACK_OFFSET (CATEYES_REMOTE_PAYLOAD_SIZE - 512)
#define CATEYES_REMOTE_DATA_FIELD(n) \
    GSIZE_TO_POINTER ((remote_address & 0xfffffffe) + CATEYES_REMOTE_DATA_OFFSET + G_STRUCT_OFFSET (CateyesTrampolineData, n))

typedef struct _CateyesInjectionInstance CateyesInjectionInstance;
typedef struct _CateyesInjectionParams CateyesInjectionParams;
typedef struct _CateyesCodeChunk CateyesCodeChunk;
typedef struct _CateyesTrampolineData CateyesTrampolineData;
typedef struct _CateyesFindLandingStripContext CateyesFindLandingStripContext;

typedef void (* CateyesEmitFunc) (const CateyesInjectionParams * params, GumAddress remote_address, CateyesCodeChunk * code);

struct _CateyesInjectionInstance
{
  CateyesQinjector * qinjector;
  guint id;
  pid_t pid;
...
struct _CateyesInjectionParams
{
  pid_t pid;
  const gchar * so_path;
...
struct _CateyesFindLandingStripContext
{
  pid_t pid;
  GumAddress result;
};

static gboolean cateyes_emit_and_remote_execute (CateyesEmitFunc func, const CateyesInjectionParams * params, GumAddress * result, GError ** error);

static void cateyes_emit_payload_code (const CateyesInjectionParams * params, GumAddress remote_address, CateyesCodeChunk * code);

static GumAddress cateyes_remote_alloc (pid_t pid, size_t size, int prot, GError ** error);
static int cateyes_remote_dealloc (pid_t pid, GumAddress address, size_t size, GError ** error);
static int cateyes_remote_pthread_create (pid_t pid, GumAddress address, GError ** error);
static int cateyes_remote_msync (pid_t pid, GumAddress remote_address, gint size, gint flags, GError ** error);
static gboolean cateyes_remote_write (pid_t pid, GumAddress remote_address, gconstpointer data, gsize size, GError ** error);
static gboolean cateyes_remote_write_fd (gint fd, GumAddress remote_address, gconstpointer data, gsize size, GError ** error);
static gboolean cateyes_remote_call (pid_t pid, GumAddress func, const GumAddress * args, gint args_length, GumAddress * retval, GError ** error);

static GumAddress cateyes_resolve_remote_libc_function (int remote_pid, const gchar * function_name);

static GumAddress cateyes_resolve_remote_library_function (int remote_pid, const gchar * library_name, const gchar * function_name);
static GumAddress cateyes_find_library_base (pid_t pid, const gchar * library_name, gchar ** library_path);

static CateyesInjectionInstance *
cateyes_injection_instance_new (CateyesQinjector * qinjector, guint id, pid_t pid, const char * temp_path)
{
  CateyesInjectionInstance * instance;
  const int mode = S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH;
  int ret;

  instance = g_slice_new0 (CateyesInjectionInstance);
...
static void
cateyes_injection_instance_free (CateyesInjectionInstance * instance)
{
  if (instance->remote_payload != 0)
  {
    GError * error = NULL;

    cateyes_remote_dealloc (instance->pid, instance->remote_payload, CATEYES_REMOTE_PAYLOAD_SIZE, &error);
  ...
  g_slice_free (CateyesInjectionInstance, instance);
}

GInputStream *
_cateyes_qinjector_get_fifo_for_instance (CateyesQinjector * self, void * instance)
{
  return g_unix_input_stream_new (((CateyesInjectionInstance *) instance)->fifo, FALSE);
}

void
_cateyes_qinjector_free_instance (CateyesQinjector * self, void * instance)
{
  cateyes_injection_instance_free (instance);
}

guint
_cateyes_qinjector_do_inject (CateyesQinjector * self, guint pid, const gchar * path, const gchar * entrypoint, const gchar * data,
    const gchar * temp_path, GError ** error)
{
  CateyesInjectionInstance * instance;
  CateyesInjectionParams params = { pid, path, entrypoint, data };

  instance = cateyes_injection_instance_new (self, self->next_instance_id++, pid, temp_path);

  params.fifo_path = instance->fifo_path;
  params.remote_address = cateyes_remote_alloc (pid, CATEYES_REMOTE_PAYLOAD_SIZE, PROT_READ | PROT_WRITE | PROT_EXEC, error);
  if (params.remote_address == 0)
    goto beach;
  instance->remote_payload = params.remote_address;

  if (!cateyes_emit_and_remote_execute (cateyes_emit_payload_code, &params, NULL, error))
    goto beach;
...
static gboolean
cateyes_emit_and_remote_execute (CateyesEmitFunc func, const CateyesInjectionParams * params, GumAddress * result,
    GError ** error)
{
  ...
  data = (CateyesTrampolineData *) (code.bytes + CATEYES_REMOTE_DATA_OFFSET);
  ...
  if (!cateyes_remote_write (params->pid, params->remote_address, code.bytes, CATEYES_REMOTE_DATA_OFFSET + sizeof (CateyesTrampolineData), error))
    return FALSE;
   ...
   */
  if (cateyes_remote_msync (params->pid, params->remote_address, CATEYES_REMOTE_PAYLOAD_SIZE, MS_SYNC | MS_INVALIDATE_ICACHE, error) != 0)
    return FALSE;

  if (cateyes_remote_pthread_create (params->pid, params->remote_address, error) != 0)
    return FALSE;
...
static void
cateyes_thumb_commit_code (GumThumbWriter * cw, CateyesCodeChunk * code)
{
...
static void
cateyes_emit_payload_code (const CateyesInjectionParams * params, GumAddress remote_address, CateyesCodeChunk * code)
{
  GumThumbWriter cw;
  GumArmWriter caw;
  ...
  gum_thumb_writer_put_call_address_with_arguments (&cw,
      cateyes_resolve_remote_libc_function (params->pid, "open"),
      2,
      GUM_ARG_ADDRESS, GUM_ADDRESS (CATEYES_REMOTE_DATA_FIELD (fifo_path)),
      GUM_ARG_ADDRESS, GUM_ADDRESS (O_WRONLY));
  gum_thumb_writer_put_mov_reg_reg (&cw, ARM_REG_R7, ARM_REG_R0);

  gum_thumb_writer_put_call_address_with_arguments (&cw,
      cateyes_resolve_remote_libc_function (params->pid, "write"),
      3,
      GUM_ARG_REGISTER, ARM_REG_R7,
      GUM_ARG_ADDRESS, GUM_ADDRESS (CATEYES_REMOTE_DATA_FIELD (entrypoint_name)),
      GUM_ARG_ADDRESS, GUM_ADDRESS (1));

  gum_thumb_writer_put_call_address_with_arguments (&cw,
      cateyes_resolve_remote_libc_function (params->pid, "dlopen"),
      2,
      GUM_ARG_ADDRESS, GUM_ADDRESS (CATEYES_REMOTE_DATA_FIELD (so_path)),
      GUM_ARG_ADDRESS, GUM_ADDRESS (RTLD_LAZY));
  gum_thumb_writer_put_mov_reg_reg (&cw, ARM_REG_R6, ARM_REG_R0);

  gum_thumb_writer_put_call_address_with_arguments (&cw,
      cateyes_resolve_remote_libc_function (params->pid, "dlsym"),
      2,
  ...
  gum_thumb_writer_put_call_reg_with_arguments (&cw,
      ARM_REG_R5,
      3,
      GUM_ARG_ADDRESS, GUM_ADDRESS (CATEYES_REMOTE_DATA_FIELD (entrypoint_data)),
      GUM_ARG_ADDRESS, GUM_ADDRESS (0),
      GUM_ARG_ADDRESS, GUM_ADDRESS (0));

  gum_thumb_writer_put_call_address_with_arguments (&cw,
      cateyes_resolve_remote_libc_function (params->pid, "dlclose"),
      1,
      GUM_ARG_REGISTER, ARM_REG_R6);

  gum_thumb_writer_put_call_address_with_arguments (&cw,
      cateyes_resolve_remote_libc_function (params->pid, "close"),
      1,
      GUM_ARG_REGISTER, ARM_REG_R7);

  gum_thumb_writer_put_pop_regs (&cw, 4, ARM_REG_R5, ARM_REG_R6, ARM_REG_R7, ARM_REG_PC);

  cateyes_thumb_commit_code (&cw, code);
...
cateyes_remote_alloc (pid_t pid, size_t size, int prot, GError ** error)
{
  ...
  GumAddress retval = 0;
  GumAddress function = cateyes_resolve_remote_libc_function (pid, "mmap");
  ...
  if (!cateyes_remote_call (pid, function, args, G_N_ELEMENTS (args), &retval, error))
    return 0;
...
cateyes_remote_dealloc (pid_t pid, GumAddress address, size_t size, GError ** error)
{
  ...
  GumAddress retval;
  GumAddress function = cateyes_resolve_remote_libc_function (pid, "munmap");
...
static int
cateyes_remote_pthread_create (pid_t pid, GumAddress remote_address, GError ** error)
{
  GumAddress args[] = {
    GPOINTER_TO_SIZE (CATEYES_REMOTE_DATA_FIELD (worker_thread)),
    GPOINTER_TO_SIZE (CATEYES_REMOTE_DATA_FIELD (create_thread_attr)),
    remote_address,
    0
  };
  GumAddress retval;
  GumAddress function = cateyes_resolve_remote_libc_function (pid, "pthread_create");
...
static int
cateyes_remote_msync (pid_t pid, GumAddress remote_address, gint size, gint flags, GError ** error)
{
  GumAddress args[] = {
    remote_address,
    size,
  ...
  GumAddress retval;
  GumAddress function = cateyes_resolve_remote_libc_function (pid, "msync");
...
cateyes_remote_write (pid_t pid, GumAddress remote_address, gconstpointer data, gsize size, GError ** error)
{
  ...
  result = cateyes_remote_write_fd (fd, remote_address, data, size, error);
...
static gboolean
cateyes_remote_write_fd (gint fd, GumAddress remote_address, gconstpointer data, gsize size, GError ** error)
{
  long ret;
...
static gboolean
cateyes_remote_call (pid_t pid, GumAddress func, const GumAddress * args, gint args_length, GumAddress * retval, GError ** error)
{
  ...
  for (i = args_length - 1; i >= 4; i--)
  {
    ...
    if (!cateyes_remote_write_fd (fd, modified_registers.arm.gpr[GUM_QNX_ARM_REG_SP], &args[i],
        4, error))
      goto beach;
  }
...
static GumAddress
cateyes_resolve_remote_libc_function (int remote_pid, const gchar * function_name)
{
  return cateyes_resolve_remote_library_function (remote_pid, "libc", function_name);
}

static GumAddress
cateyes_resolve_remote_library_function (int remote_pid, const gchar * library_name, const gchar * function_name)
{
  ...
  remote_base = cateyes_find_library_base (remote_pid, library_name, &remote_library_path);
  if (remote_base == -1)
    return -1;
