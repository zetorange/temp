...
#endif
  SCRIPT_TESTENTRY (cateyes_version_is_available)
  SCRIPT_TESTENTRY (cateyes_heap_size_can_be_queried)
...
SCRIPT_TESTCASE (inline_sqlite_database_can_be_queried)
{
  ...
  EXPECT_SEND_MESSAGE_WITH ("null");
  EXPECT_SEND_MESSAGE_WITH ("[\"Cateyes\",7]");
  ...
  /* 4: bindBlob() */
  EXPECT_SEND_MESSAGE_WITH ("[\"Cateyes\"]");
  EXPECT_SEND_MESSAGE_WITH ("null");
...
SCRIPT_TESTCASE (external_sqlite_database_can_be_queried)
{
  ...
  COMPILE_AND_LOAD_SCRIPT (
      ...
      "db.exec(\""
          ...
          "INSERT INTO people VALUES (2, 'Cateyes', 7, 140, X'1337');"
          "COMMIT;"
      "\");\n"
      "send(db.dump());\n"
      "db.close();\n");
...
SCRIPT_TESTCASE (socket_connection_can_be_established)
{
...
#ifdef G_OS_UNIX
  {
    ...
    COMPILE_AND_LOAD_SCRIPT (
        ...
        "Socket.listen({"
        "  type: 'path',"
        "  path: '%s/cateyes-gum-test-listener-' + getpid(),"
        "  backlog: 1,"
        "})"
        ".then(function (listener) {"
        "  listener.accept()"
...
SCRIPT_TESTCASE (cateyes_version_is_available)
{
  COMPILE_AND_LOAD_SCRIPT ("send(typeof Cateyes.version);");
  EXPECT_SEND_MESSAGE_WITH ("\"string\"");
}

SCRIPT_TESTCASE (cateyes_heap_size_can_be_queried)
{
  COMPILE_AND_LOAD_SCRIPT ("send(typeof Cateyes.heapSize);");
  EXPECT_SEND_MESSAGE_WITH ("\"number\"");
}
...
SCRIPT_TESTCASE (rpc_can_be_performed)
{
  ...
  POST_MESSAGE ("[\"cateyes:rpc\",1,\"list\"]");
  EXPECT_SEND_MESSAGE_WITH ("[\"cateyes:rpc\",1,\"ok\","
      "[\"foo\",\"bar\",\"badger\"]]");

  POST_MESSAGE ("[\"cateyes:rpc\",2,\"call\",\"foo\",[1,2]]");
  EXPECT_SEND_MESSAGE_WITH ("[\"cateyes:rpc\",2,\"ok\",3]");

  POST_MESSAGE ("[\"cateyes:rpc\",3,\"call\",\"foo\",[1,-2]]");
  EXPECT_SEND_MESSAGE_WITH_PREFIX ("[\"cateyes:rpc\",3,\"error\",\"no\",");

  POST_MESSAGE ("[\"cateyes:rpc\",4,\"call\",\"bar\",[3,4]]");
  EXPECT_SEND_MESSAGE_WITH ("[\"cateyes:rpc\",4,\"ok\",7]");

  POST_MESSAGE ("[\"cateyes:rpc\",5,\"call\",\"bar\",[3,-4]]");
  EXPECT_SEND_MESSAGE_WITH_PREFIX ("[\"cateyes:rpc\",5,\"error\",\"nope\",");

  POST_MESSAGE ("[\"cateyes:rpc\",6,\"call\",\"baz\",[]]");
  EXPECT_SEND_MESSAGE_WITH ("[\"cateyes:rpc\",6,\"error\","
      "\"unable to find method 'baz'\"]");

  POST_MESSAGE ("[\"cateyes:rpc\",7,\"call\",\"badger\",[]]");
  EXPECT_SEND_MESSAGE_WITH_PAYLOAD_AND_DATA ("[\"cateyes:rpc\",7,\"ok\",{}]",
      "59 6f");
}
...
SCRIPT_TESTCASE (source_maps_should_be_supported_for_our_runtime)
{
  ...
  item = test_script_fixture_pop_message (fixture);
  g_assert (strstr (item->message, " (cateyes/runtime/hexdump.js:") != NULL);
  test_script_message_item_free (item);
...
SCRIPT_TESTCASE (source_maps_should_be_supported_for_user_scripts)
{
  ...
  if (GUM_DUK_IS_SCRIPT_BACKEND (fixture->backend))
  {
    g_assert (strstr (item->message,
        ...
        "    at math.js:5\\n"
        "    at index.js:6\\n"
        "    at s (node_modules/cateyes/node_modules/browserify/node_modules/"
            "browser-pack/_prelude.js:1)\\n"
        "    at e (node_modules/cateyes/node_modules/browserify/node_modules/"
            "browser-pack/_prelude.js:1)\\n")
        != NULL);
  }
  else
  {
    g_assert (strstr (item->message,
        ...
        "    at e (node_modules/cateyes/node_modules/browserify/node_modules/"
            "browser-pack/_prelude.js:1:1)\\n"
        "    at node_modules/cateyes/node_modules/browserify/node_modules/"
