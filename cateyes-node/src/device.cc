...
Device::Device(CateyesDevice* handle, Runtime* runtime)
...
NAN_METHOD(Device::New) {
  ...
  auto handle = static_cast<CateyesDevice*>(
...
NAN_PROPERTY_GETTER(Device::GetId) {
  auto handle = ObjectWrap::Unwrap<Device>(
      info.Holder())->GetHandle<CateyesDevice>();

  info.GetReturnValue().Set(
      Nan::New(cateyes_device_get_id(handle)).ToLocalChecked());
}
...
NAN_PROPERTY_GETTER(Device::GetName) {
  ...
  info.GetReturnValue().Set(
      Nan::New(cateyes_device_get_name(handle)).ToLocalChecked());
}

NAN_PROPERTY_GETTER(Device::GetIcon) {
  auto wrapper = ObjectWrap::Unwrap<Device>(info.Holder());
  auto handle = wrapper->GetHandle<CateyesDevice>();

  info.GetReturnValue().Set(Icon::New(cateyes_device_get_icon(handle),
      wrapper->runtime_));
}
...
NAN_PROPERTY_GETTER(Device::GetType) {
  ...
  info.GetReturnValue().Set(Runtime::ValueFromEnum(
      cateyes_device_get_dtype(handle), CATEYES_TYPE_DEVICE_TYPE));
}

class GetFrontmostApplicationOperation : public Operation<CateyesDevice> {
 public:
  void Begin() {
    cateyes_device_get_frontmost_application(handle_, OnReady, this);
  }

  void End(GAsyncResult* result, GError** error) {
    application_ = cateyes_device_get_frontmost_application_finish(handle_,
        result, error);
  }
  ...
  CateyesApplication* application_;
};
...
class EnumerateApplicationsOperation : public Operation<CateyesDevice> {
 public:
  void Begin() {
    cateyes_device_enumerate_applications(handle_, OnReady, this);
  }

  void End(GAsyncResult* result, GError** error) {
    applications_ = cateyes_device_enumerate_applications_finish(handle_, result,
        error);
  }

  Local<Value> Result(Isolate* isolate) {
    auto size = cateyes_application_list_size(applications_);
    auto applications = Nan::New<v8::Array>(size);
    for (auto i = 0; i != size; i++) {
      auto handle = cateyes_application_list_get(applications_, i);
  ...
  CateyesApplicationList* applications_;
};
...
class EnumerateProcessesOperation : public Operation<CateyesDevice> {
 public:
  void Begin() {
    cateyes_device_enumerate_processes(handle_, OnReady, this);
  }

  void End(GAsyncResult* result, GError** error) {
    processes_ = cateyes_device_enumerate_processes_finish(handle_, result,
        error);
  }

  Local<Value> Result(Isolate* isolate) {
    auto size = cateyes_process_list_size(processes_);
    auto processes = Nan::New<v8::Array>(size);
    for (auto i = 0; i != size; i++) {
      auto handle = cateyes_process_list_get(processes_, i);
...
class EnableSpawnGatingOperation : public Operation<CateyesDevice> {
 public:
  void Begin() {
    cateyes_device_enable_spawn_gating(handle_, OnReady, this);
  }

  void End(GAsyncResult* result, GError** error) {
    cateyes_device_enable_spawn_gating_finish(handle_, result, error);
  }
...
class DisableSpawnGatingOperation : public Operation<CateyesDevice> {
 public:
  void Begin() {
    cateyes_device_disable_spawn_gating(handle_, OnReady, this);
  }

  void End(GAsyncResult* result, GError** error) {
    cateyes_device_disable_spawn_gating_finish(handle_, result, error);
  }
...
class EnumeratePendingSpawnOperation : public Operation<CateyesDevice> {
 public:
  void Begin() {
    cateyes_device_enumerate_pending_spawn(handle_, OnReady, this);
  }

  void End(GAsyncResult* result, GError** error) {
    pending_spawn_ = cateyes_device_enumerate_pending_spawn_finish(handle_,
        result, error);
  }

  Local<Value> Result(Isolate* isolate) {
    auto size = cateyes_spawn_list_size(pending_spawn_);
    auto pending_spawn = Nan::New<v8::Array>(size);
    for (auto i = 0; i != size; i++) {
      auto handle = cateyes_spawn_list_get(pending_spawn_, i);
  ...
  CateyesSpawnList* pending_spawn_;
};
...
class EnumeratePendingChildrenOperation : public Operation<CateyesDevice> {
 public:
  void Begin() {
    cateyes_device_enumerate_pending_children(handle_, OnReady, this);
  }

  void End(GAsyncResult* result, GError** error) {
    pending_children_ = cateyes_device_enumerate_pending_children_finish(handle_,
        result, error);
  }

  Local<Value> Result(Isolate* isolate) {
    auto size = cateyes_child_list_size(pending_children_);
    auto pending_children = Nan::New<v8::Array>(size);
    for (auto i = 0; i != size; i++) {
      auto handle = cateyes_child_list_get(pending_children_, i);
  ...
  CateyesChildList* pending_children_;
};
...
class SpawnOperation : public Operation<CateyesDevice> {
 public:
  SpawnOperation(gchar* program, CateyesSpawnOptions* options)
    : program_(program),
      options_(options) {
  }
  ...
  void Begin() {
    cateyes_device_spawn(handle_, program_, options_, OnReady, this);
  }

  void End(GAsyncResult* result, GError** error) {
    pid_ = cateyes_device_spawn_finish(handle_, result, error);
  }
...
NAN_METHOD(Device::Spawn) {
  ...
  auto options = cateyes_spawn_options_new();
  bool valid = true;
  ...
  if (valid && !stdio_value->IsNull()) {
    CateyesStdio stdio;
    valid = Runtime::ValueToEnum(stdio_value, CATEYES_TYPE_STDIO, &stdio);
    if (valid)
  ...
  if (valid) {
    if (aux_value->IsObject()) {
      ...
      GVariantDict* aux = cateyes_spawn_options_get_aux(options);
...
class InputOperation : public Operation<CateyesDevice> {
 public:
  ...
  void Begin() {
    cateyes_device_input(handle_, pid_, data_, OnReady, this);
  }

  void End(GAsyncResult* result, GError** error) {
    cateyes_device_input_finish(handle_, result, error);
  }
...
class ResumeOperation : public Operation<CateyesDevice> {
 public:
  ResumeOperation(guint pid) : pid_(pid) {
  }

  void Begin() {
    cateyes_device_resume(handle_, pid_, OnReady, this);
  }

  void End(GAsyncResult* result, GError** error) {
    cateyes_device_resume_finish(handle_, result, error);
  }
...
class KillOperation : public Operation<CateyesDevice> {
 public:
  KillOperation(guint pid) : pid_(pid) {
  }

  void Begin() {
    cateyes_device_kill(handle_, pid_, OnReady, this);
  }

  void End(GAsyncResult* result, GError** error) {
    cateyes_device_kill_finish(handle_, result, error);
  }
...
class AttachOperation : public Operation<CateyesDevice> {
 public:
  AttachOperation(guint pid) : pid_(pid) {
  }

  void Begin() {
    cateyes_device_attach(handle_, pid_, OnReady, this);
  }

  void End(GAsyncResult* result, GError** error) {
    session_ = cateyes_device_attach_finish(handle_, result, error);
  }
...
class InjectLibraryFileOperation : public Operation<CateyesDevice> {
 public:
  ...
  void Begin() {
    cateyes_device_inject_library_file(handle_, pid_, path_, entrypoint_, data_,
        OnReady, this);
  }

  void End(GAsyncResult* result, GError** error) {
    id_ = cateyes_device_inject_library_file_finish(handle_, result, error);
  }
...
class InjectLibraryBlobOperation : public Operation<CateyesDevice> {
 public:
  ...
  void Begin() {
    cateyes_device_inject_library_blob(handle_, pid_, blob_, entrypoint_, data_,
        OnReady, this);
  }

  void End(GAsyncResult* result, GError** error) {
    id_ = cateyes_device_inject_library_blob_finish(handle_, result, error);
  }
