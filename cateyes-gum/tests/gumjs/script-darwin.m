...
@interface CateyesDefaultCalculator : NSObject<CateyesCalculator> {
  NSString * name;
}
@end

@implementation CateyesDefaultCalculator
- (id)init {
  self = [super init];
  if (self) {
    self->name = @"calc.exe";
...
SCRIPT_TESTCASE (object_enumeration_should_contain_protocol_methods)
{
  @autoreleasepool
  {
    CateyesDefaultCalculator * calc = [[[CateyesDefaultCalculator alloc] init]
        autorelease];
    ...
    COMPILE_AND_LOAD_SCRIPT (
        ...
        "var calculator = new ObjC.Object(calculatorProxy, "
            "ObjC.protocols.CateyesCalculator);"
...
SCRIPT_TESTCASE (class_can_be_implemented)
{
  @autoreleasepool
  {
    const gchar * class_name = "CateyesJSCalculator";

    COMPILE_AND_LOAD_SCRIPT (
        "var CateyesJSCalculator = ObjC.registerClass({"
            "name: \"%s\","
            "super: ObjC.classes.NSObject,"
            "protocols: [ObjC.protocols.CateyesCalculator],"
            "methods: {"
                "\"- init\": function () {"
             ...
             "}"
        "});"
        "send(CateyesJSCalculator.$className === \"%s\");",
...
SCRIPT_TESTCASE (block_can_be_migrated_to_the_heap_behind_our_back)
{
  @autoreleasepool
  {
    ...
    COMPILE_AND_LOAD_SCRIPT (
        "var CateyesDefaultCalculator = ObjC.classes.CateyesDefaultCalculator;"
        "var m = CateyesDefaultCalculator['- add:completion:'].implementation;"
...
typedef struct _CateyesRect CateyesRect;
typedef struct _CateyesPoint CateyesPoint;
typedef struct _CateyesSize CateyesSize;

struct _CateyesPoint
{
  double x;
  double y;
};
...
@interface CateyesWidget : NSObject
@end

@implementation CateyesWidget

- (CateyesPoint)position {
  CateyesPoint p;
  p.x = 10.0;
  p.y = 15.0;
  return p;
...
- (int)drawRect:(CateyesRect)dirtyRect {
...
SCRIPT_TESTCASE (struct_consuming_method_implementation_can_be_overridden)
{
  @autoreleasepool
  {
    CateyesWidget * widget = [[[CateyesWidget alloc] init] autorelease];
    CateyesRect r;

    COMPILE_AND_LOAD_SCRIPT (
        "var CateyesWidget = ObjC.classes.CateyesWidget;"
        "var method = CateyesWidget[\"- drawRect:\"];"
...
@interface CateyesTest1 : NSObject
+ (int)foo_;
+ (int)fooBar_;
+ (int)fooBar:(int)a;
+ (int):(int)a;
...
SCRIPT_TESTCASE (methods_with_weird_names_can_be_invoked)
{
  ...
  {
    COMPILE_AND_LOAD_SCRIPT (
        "var CateyesTest1 = ObjC.classes.CateyesTest1;"
        "var methodNames = ['foo_', 'fooBar_', 'fooBar:', ':', '::'];"
        "var args = [0, 0, 1, 1, 2];"
        "for (var i = 0; i < methodNames.length; i++) {"
            "var m = CateyesTest1['+ ' + methodNames[i]];"
            "var val = m.apply(CateyesTest1, args[i] == 0? []: args[i] == 1? [0]: [0, 0]);"
            "send(val == i + 1);"
        "}");
...
@interface CateyesTest2 : NSObject
@end

#define METHOD(t, n) + (t)_ ## n:(t)x { return x; }
@implementation CateyesTest2
METHOD(char, char)
METHOD(int, int)
METHOD(short, short)
METHOD(long, long)
...
SCRIPT_TESTCASE (method_call_preserves_value)
{
  ...
  {
    COMPILE_AND_LOAD_SCRIPT (
        "var CateyesTest2 = ObjC.classes.CateyesTest2;"
        "function test(method, value) {"
            "var arg_value = value;"
            "if (typeof value === 'string') {"
                "arg_value = Memory.allocUtf8String(value);"
            "}"
            "var result = CateyesTest2['+ _' + method + ':'](arg_value);"
        ...
        "test('char_ptr', 'cateyes');"
        "test('id', CateyesTest2);"
        "test('id', ObjC.classes.NSObject.new());"
        "test('Class', CateyesTest2);"
...
@interface CateyesTest3 : NSObject
@end
@implementation CateyesTest3
@end

@interface CateyesTest4 : CateyesTest3
@end
@implementation CateyesTest4
@end
...
SCRIPT_TESTCASE (objects_can_be_chosen)
{
  ...
  @autoreleasepool
  {
    COMPILE_AND_LOAD_SCRIPT (
        ...
        "}"
        "var CateyesTest3 = ObjC.classes.CateyesTest3;"
        "var CateyesTest4 = ObjC.classes.CateyesTest4;"
        "var obj3 = CateyesTest3.new();"
        "var obj4 = CateyesTest4.new();"
        "send(testChoose(obj3, CateyesTest3));"
        "send(!testChoose(obj3, CateyesTest4));"
        "send(testChoose(obj4, CateyesTest3));"
        "send(testChoose(obj4, CateyesTest4));"
        "send(testChoose(obj3, CateyesTest3, true));"
        "send(!testChoose(obj3, CateyesTest4, true));"
        "send(!testChoose(obj4, CateyesTest3, true));"
        "send(testChoose(obj4, CateyesTest4, true));");
...
{
  COMPILE_AND_LOAD_SCRIPT (
      "var cateyesThreadId = Process.getCurrentThreadId();"
      "ObjC.schedule(" GUM_PTR_CONST ", function () {"
          "send(Process.getCurrentThreadId() !== cateyesThreadId);"
