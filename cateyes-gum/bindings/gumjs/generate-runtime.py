...
def to_canonical_source_path(path):
    return os.path.join("cateyes", path).replace("\\", "/")
...
def identifier(filename):
    result = ""
    if filename.startswith("cateyes-"):
        filename = filename[6:]
    for c in filename:
        if c.isalnum():
            result += c.lower()
...
if __name__ == '__main__':
    ...
    runtime = os.path.abspath(os.path.join(v8_tmp_dir, "cateyes.js"))
    objc = os.path.abspath(os.path.join(v8_tmp_dir, "objc.js"))
    java = os.path.abspath(os.path.join(v8_tmp_dir, "java.js"))

    print('---')
    print(node_script_path("cateyes-compile"))
    print(runtime)
    print(input_dir)
    print('---')
    subprocess.check_call([node_script_path("cateyes-compile"), "./runtime/entrypoint-v8.js", "-o", runtime, "-x"], cwd=input_dir)
    subprocess.check_call([node_script_path("cateyes-compile"), "./runtime/objc.js", "-o", objc, "-x"], cwd=input_dir)
    subprocess.check_call([node_script_path("cateyes-compile"), "./runtime/java.js", "-o", java, "-x"], cwd=input_dir)
    ...
    generate_runtime_v8("debug", output_dir, "gumv8script-debug.h", [os.path.join(input_dir, "cateyes-debug.js")])

    duk_tmp_dir = os.path.join(output_dir, "runtime-build-duk")
    runtime = os.path.abspath(os.path.join(duk_tmp_dir, "cateyes.js"))
    ...
    subprocess.check_call([node_script_path("cateyes-compile"), "./runtime/entrypoint-duktape.js", "-o", runtime, "-c"], cwd=input_dir)
    subprocess.check_call([node_script_path("cateyes-compile"), "./runtime/promise.js", "-o", promise, "-c"], cwd=input_dir)
    subprocess.check_call([node_script_path("cateyes-compile"), "./runtime/objc.js", "-o", objc, "-c"], cwd=input_dir)
    subprocess.check_call([node_script_path("cateyes-compile"), "./runtime/java.js", "-o", java, "-c"], cwd=input_dir)
