...
typedef struct _CateyesInitMessage CateyesInitMessage;

struct _CateyesInitMessage
{
...
gint
_cateyes_darwin_pipe_consume_stashed_file_descriptor (const gchar * address, GError ** error)
{
  gint fd = -1;
  gint assigned;
  mach_port_t port = MACH_PORT_NULL;
  CateyesInitMessage init = { { 0, }, { 0, } };
  kern_return_t kr;
  const gchar * failed_operation;
  mach_port_t wrapper;
