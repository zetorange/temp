...
def main():
    ...
    from cateyes_tools.application import ConsoleApplication
    ...
    class REPLApplication(ConsoleApplication):
        def __init__(self):
            ...
            self._completer = CateyesCompleter(self)
            self._cli = None
        ...
        def _monitor_script(self):
            ...
            if self._user_script is not None:
                monitor = cateyes.FileMonitor(self._user_script)
        ...
        def _process_input(self, reactor):
            ...
            while True:
                ...
                if expression.endswith("?"):
                    ...
                    except cateyes.InvalidOperationError:
                        return
        ...
        def _print_startup_message(self):
            self._print("""\
     ____
    / _  |   Cateyes {version} - A world-class dynamic instrumentation toolkit
   | (_| |
    > _  |   Commands:
   ...
   . . . .       exit/quit -> Exit
   . . . .
   . . . .   More info at http://www.cateyes.re/docs/home/""".format(version=cateyes.__version__))
...
"""
        ...
        def _load_codeshare_script(self, uri):
            ...
            project_url = "https://codeshare.cateyes.re/api/project/{}/".format(uri)
            response_json = None
            try:
                request = build_opener()
                request.addheaders = [('User-Agent', 'Cateyes v{} | {}'.format(cateyes.__version__, platform.platform()))]
...
URL: {url}
            """.format(
                ...
                slug=uri,
                fingerprint=fingerprint,
                url="https://codeshare.cateyes.re/@{}".format(uri)
            ))
        ...
        def _get_or_create_config_dir(self):
            config_dir = os.path.join(os.path.expanduser('~'), '.cateyes')
    ...
    class CateyesCompleter(Completer):
