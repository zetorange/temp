...
Spawn::Spawn(CateyesSpawn* handle, Runtime* runtime)
...
NAN_METHOD(Spawn::New) {
  ...
  auto handle = static_cast<CateyesSpawn*>(
...
NAN_PROPERTY_GETTER(Spawn::GetPid) {
  auto handle = ObjectWrap::Unwrap<Spawn>(
      info.Holder())->GetHandle<CateyesSpawn>();

  info.GetReturnValue().Set(Nan::New<Integer>(cateyes_spawn_get_pid(handle)));
}
...
NAN_PROPERTY_GETTER(Spawn::GetIdentifier) {
  ...
  auto identifier = cateyes_spawn_get_identifier(handle);
  if (identifier != NULL)
