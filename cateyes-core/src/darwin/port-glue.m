...
typedef struct _CateyesHandshakeMessageOut CateyesHandshakeMessageOut;
typedef struct _CateyesHandshakeMessageIn CateyesHandshakeMessageIn;

struct _CateyesHandshakeMessageOut
{
...
struct _CateyesHandshakeMessageIn
{
  mach_msg_base_t base;
  mach_msg_port_descriptor_t task;
  mach_msg_port_descriptor_t socket;
...
static pid_t cateyes_audit_token_to_pid (audit_token_t atoken);
...
guint
_cateyes_handshake_port_create_local (CateyesHandshakePort * self, const gchar * name, GError ** error)
{
...
guint
_cateyes_handshake_port_create_remote (CateyesHandshakePort * self, const gchar * name, GError ** error)
{
...
void
_cateyes_handshake_port_deallocate (CateyesHandshakePort * self)
{
...
void
_cateyes_handshake_port_perform_exchange_as_sender (CateyesHandshakePort * self, guint * task_port, gint * fd, GError ** error)
{
  mach_port_t self_task, local_rx;
  CateyesHandshakeMessageOut msg_out;
  CateyesHandshakeMessageIn msg_in;
...
void
_cateyes_handshake_port_perform_exchange_as_receiver (CateyesHandshakePort * self, guint peer_pid, guint * task_port, gint * fd, GError ** error)
{
  mach_port_t self_task, local_rx;
  CateyesHandshakeMessageIn msg_in;
  CateyesHandshakeMessageOut msg_out;
  ...
  if (header_in->msgh_size != sizeof (CateyesHandshakeMessageOut))
    goto handshake_failed;
  if (cateyes_audit_token_to_pid (msg_in.trailer.msgh_audit) != peer_pid)
    goto handshake_failed;
...
void
_cateyes_task_port_deallocate (CateyesTaskPort * self)
{
  mach_port_deallocate (mach_task_self (), cateyes_task_port_get_mach_port (self));
}

static pid_t
cateyes_audit_token_to_pid (audit_token_t atoken)
{
#ifdef HAVE_MACOS
