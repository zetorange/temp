...
static mach_port_t cateyes_jit = MACH_PORT_NULL;
#endif
...
{
#ifdef HAVE_IOS
  bootstrap_look_up (bootstrap_port, "com.apple.uikit.viewservice.cateyes",
      &cateyes_jit);
#endif
}
...
{
#ifdef HAVE_IOS
  if (cateyes_jit != MACH_PORT_NULL)
  {
    mach_port_deallocate (mach_task_self (), cateyes_jit);
    cateyes_jit = MACH_PORT_NULL;
  }
#endif
}
                   ...
                   GumPageProtection page_prot)
{
  ...
  if (using_jit_server)
  {
    kr = cateyes_jit_alloc (cateyes_jit, mach_task_self (), &result, size,
        VM_FLAGS_ANYWHERE);
  }
  else
#endif
                            ...
                            const GumAddressSpec * address_spec)
{
  ...
  ctx.using_jit_server = (page_prot == GUM_PAGE_RWX &&
      !gum_query_is_rwx_supported () && cateyes_jit != MACH_PORT_NULL);
#else
                                       ...
                                       gpointer user_data)
{
  ...
  {
    kr = cateyes_jit_alloc (cateyes_jit, ctx->task, &address, ctx->size,
        VM_FLAGS_FIXED);
  }
  else
#endif
