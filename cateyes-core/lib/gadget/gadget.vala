	...
	private class Script : Object {
		...
		private bool try_handle_rpc_message (Json.Object message) {
			...
			if (rpc_message.get_length () < 4)
				return false;
			else if (rpc_message.get_element (0).get_string () != "cateyes:rpc")
				return false;
	...
	private class Server : Object, Controller {
		...
		private class Client : Object, HostSession {
			...
			public Client (Server s, DBusConnection c) {
				...
				try {
					host_registration_id = connection.register_object (Cateyes.ObjectPath.HOST_SESSION, this as HostSession);
				} catch (IOError e) {
					assert_not_reached ();
				}

				var pid = _getpid ();
				var identifier = "re.cateyes.Gadget";
				var name = "Gadget";
