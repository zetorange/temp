namespace Cateyes.Server {
...
#if DARWIN
		var worker = new Thread<int> ("cateyes-server-main-loop", () => {
	...
	public class Application : Object {
		...
		construct {
			TemporaryDirectory.always_use ("re.cateyes.server");
		...
		private class Client : Object {
			...
			private GLib.DBusMessage on_connection_message (DBusConnection connection, owned DBusMessage message, bool incoming) {
				...
				if (iface == "re.cateyes.HostSession12") {
				...
				} else if (iface == "re.cateyes.AgentSession12") {
					uint session_id;
					path.scanf ("/re/cateyes/AgentSession/%u", out session_id);
					if (member == "Close") {
