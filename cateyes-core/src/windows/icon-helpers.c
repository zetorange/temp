...
CateyesImageData *
_cateyes_image_data_from_process_or_file (DWORD pid, WCHAR * filename, CateyesIconSize size)
{
  CateyesImageData * icon;

  icon = _cateyes_image_data_from_process (pid, size);
  if (icon == NULL)
    icon = _cateyes_image_data_from_file (filename, size);

  if (icon == NULL)
  {
    icon = g_new (CateyesImageData, 1);
    cateyes_image_data_init (icon, 0, 0, 0, "");
  }
...
CateyesImageData *
_cateyes_image_data_from_process (DWORD pid, CateyesIconSize size)
{
  ...
  if (main_window != NULL)
  {
    ...
    else if (size == CATEYES_ICON_LARGE)
    {
  ...
  if (icon != NULL)
    result = _cateyes_image_data_from_native_icon_handle (icon, size);

  return result;
}

CateyesImageData *
_cateyes_image_data_from_file (WCHAR * filename, CateyesIconSize size)
{
  CateyesImageData * result = NULL;
  SHFILEINFO shfi = { 0, };
  UINT flags;
  ...
  if (shfi.hIcon != NULL)
    result = _cateyes_image_data_from_native_icon_handle (shfi.hIcon, size);

  return result;
}

CateyesImageData *
_cateyes_image_data_from_resource_url (WCHAR * resource_url, CateyesIconSize size)
{
  ...
  *p = L'\0';
  ...
  ret = ExtractIconExW (resource_file, resource_id, (size == CATEYES_ICON_LARGE) ? &icon : NULL, (size == CATEYES_ICON_SMALL) ? &icon : NULL, 1);
...
CateyesImageData *
_cateyes_image_data_from_native_icon_handle (HICON icon, CateyesIconSize size)
{
  CateyesImageData * result;
  HDC dc;
  ...
  result = g_new (CateyesImageData, 1);
  data_base64 = g_base64_encode (data, rowstride * height);
  cateyes_image_data_init (result, width, height, width * 4, data_base64);
  g_free (data_base64);
