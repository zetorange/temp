...
if in_source_package:
    with codecs.open(pkg_info, "r", 'utf-8') as f:
        ...
        cateyes_version = version_line[9:]
    long_description = None
else:
    cateyes_version = os.environ['CATEYES_VERSION']
    long_description = codecs.open(os.path.join(package_dir, "README.md"), "r", 'utf-8').read()
    cateyes_extension = os.environ['CATEYES_EXTENSION']
cateyes_major_version = int(cateyes_version.split(".")[0])
...
class CateyesPrebuiltExt(build_ext):
    def build_extension(self, ext):
        ...
        if in_source_package:
            ...
            try:
                ...
                urls = client.release_urls("cateyes", cateyes_version)
                ...
                if len(urls) == 0:
                    raise Exception("Could not find prebuilt Cateyes extension. "
            ...
            if network_error is not None:
                ...
                egg_path = os.path.expanduser("~/cateyes-{}-py{}.{}-{}.egg".format(cateyes_version, python_version[0], python_version[1], os_version))
            ...
            shutil.copyfile(cateyes_extension, target)
...
setup(
    ...
    packages=['cateyes'],
    ext_modules=[Extension('_cateyes', [])],
    cmdclass={
        'build_ext': CateyesPrebuiltExt
    }
)
