...
os_failure:
  {
    ...
    else
    {
      g_set_error (error,
          CATEYES_ERROR,
          (os_error == ERROR_ACCESS_DENIED) ? CATEYES_ERROR_PERMISSION_DENIED : CATEYES_ERROR_NOT_SUPPORTED,
...
nt_failure:
  {
    ...
    if (nt_status == 0xC0000022) /* STATUS_ACCESS_DENIED */
      code = CATEYES_ERROR_PERMISSION_DENIED;
    else
      code = CATEYES_ERROR_NOT_SUPPORTED;
