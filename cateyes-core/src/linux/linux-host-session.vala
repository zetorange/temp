namespace Cateyes {
	...
	public class LinuxHostSession : BaseDBusHostSession {
		...
		construct {
			...
			var blob32 = Cateyes.Data.Agent.get_cateyes_agent_32_so_blob ();
			var blob64 = Cateyes.Data.Agent.get_cateyes_agent_64_so_blob ();
			agent = new AgentResource ("cateyes-agent-%u.so",
		...
		protected override async Gee.Promise<IOStream> perform_attach_to (uint pid, out Object? transport) throws Error {
			...
			var linjector = injector as Linjector;
			var id = yield linjector.inject_library_resource (pid, agent, "cateyes_agent_main", t.remote_address);
	...
	private class SystemUIAgent : InternalAgent {
		public SystemUIAgent (LinuxHostSession host_session) {
			string * source = Cateyes.Data.Android.get_systemui_js_blob ().data;
