...
Script::Script(CateyesScript* handle, Runtime* runtime)
...
NAN_METHOD(Script::New) {
  ...
  auto handle = static_cast<CateyesScript*>(
  ...
  auto monitor =
      new UsageMonitor<CateyesScript>(cateyes_script_is_destroyed, "destroyed");
  monitor->Enable(wrapper);

  info.GetReturnValue().Set(obj);
}

class LoadOperation : public Operation<CateyesScript> {
 public:
  void Begin() {
    cateyes_script_load(handle_, OnReady, this);
  }

  void End(GAsyncResult* result, GError** error) {
    cateyes_script_load_finish(handle_, result, error);
  }
...
class UnloadOperation : public Operation<CateyesScript> {
 public:
  void Begin() {
    cateyes_script_unload(handle_, OnReady, this);
  }

  void End(GAsyncResult* result, GError** error) {
    cateyes_script_unload_finish(handle_, result, error);
  }
...
class PostOperation : public Operation<CateyesScript> {
 public:
  ...
  void Begin() {
    cateyes_script_post(handle_, message_, data_, OnReady, this);
  }

  void End(GAsyncResult* result, GError** error) {
    cateyes_script_post_finish(handle_, result, error);
  }
