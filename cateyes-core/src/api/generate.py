...
def emit_header(api, output_dir):
    with open(os.path.join(output_dir, 'cateyes-core.h'), 'wt') as output_header_file:
        output_header_file.write("#ifndef __CATEYES_CORE_H__\n#define __CATEYES_CORE_H__\n\n")
        ...
        output_header_file.write("\n\n/* Library lifetime */")
        output_header_file.write("\nvoid cateyes_init (void);")
        output_header_file.write("\nvoid cateyes_shutdown (void);")
        output_header_file.write("\nvoid cateyes_deinit (void);")
        output_header_file.write("\nGMainContext * cateyes_get_main_context (void);")

        output_header_file.write("\n\n/* Object lifetime */")
        output_header_file.write("\nvoid cateyes_unref (gpointer obj);")

        output_header_file.write("\n\n/* Library versioning */")
        output_header_file.write("\nvoid cateyes_version (guint * major, guint * minor, guint * micro, guint * nano);")
        output_header_file.write("\nconst gchar * cateyes_version_string (void);")
        ...
        if len(api.error_types) > 0:
            output_header_file.write("\n\n/* Errors */\n")
            output_header_file.write("\n\n".join(map(lambda enum: "GQuark cateyes_%(name_lc)s_quark (void);\n" \
        ...
        macros = []
        for enum in api.enum_types:
            macros.append("#define CATEYES_TYPE_%(name_uc)s (cateyes_%(name_lc)s_get_type ())" \
                % { 'name_lc': enum.name_lc, 'name_uc': enum.name_uc })
        for object_type in api.object_types:
            macros.append("""#define CATEYES_TYPE_%(name_uc)s (cateyes_%(name_lc)s_get_type ())
#define CATEYES_%(name_uc)s(obj) (G_TYPE_CHECK_INSTANCE_CAST ((obj), CATEYES_TYPE_%(name_uc)s, Cateyes%(name)s))
#define CATEYES_IS_%(name_uc)s(obj) (G_TYPE_CHECK_INSTANCE_TYPE ((obj), CATEYES_TYPE_%(name_uc)s))""" \
                % { 'name': object_type.name, 'name_lc': object_type.name_lc, 'name_uc': object_type.name_uc })

        for enum in api.error_types:
            macros.append("#define CATEYES_%(name_uc)s (cateyes_%(name_lc)s_quark ())" \
                % { 'name_lc': enum.name_lc, 'name_uc': enum.name_uc })
        output_header_file.write("\n" + "\n\n".join(macros))
...
def emit_vapi(api, output_dir):
    with open(os.path.join(output_dir, "cateyes-core-{0}.vapi".format(api.version)), "wt") as output_vapi_file:
        output_vapi_file.write("[CCode (cheader_filename = \"cateyes-core.h\", cprefix = \"Cateyes\", lower_case_cprefix = \"cateyes_\")]")
    ...
    with open(os.path.join(output_dir, "cateyes-core-{0}.deps".format(api.version)), "wt") as output_deps_file:
...
def parse_api(api_version, api_vala, core_vapi, core_header, interfaces_vapi, interfaces_header):
            ...
            if method_cname_lc not in seen_cfunctions:
                ...
                if method_name not in ('construct', 'get_main_context', 'get_provider', 'get_session'):
                    if (object_type.c_name + '*') in m.group(0):
                        if method_name == 'new':
                            if not object_type.name.endswith("List") and (method_cprototype.endswith("(void);") or method_cprototype.startswith("CateyesFileMonitor")):
    ...
    for line in core_vapi.split("\n"):
        ...
        if level == 0:
            ...
            if stripped_line.startswith("public " + current_object_type.name + " (") or stripped_line.startswith("public static Cateyes." + current_object_type.name + " @new ("):
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="Generate refined Cateyes API definitions")
    ...
    parser.add_argument('api_vala', metavar='/path/to/cateyes.vala', type=argparse.FileType('r'))
    parser.add_argument('core_vapi', metavar='/path/to/cateyes-core.vapi', type=argparse.FileType('r'))
    parser.add_argument('core_header', metavar='/path/to/cateyes-core.h', type=argparse.FileType('r'))
    parser.add_argument('interfaces_vapi', metavar='/path/to/cateyes-interfaces.vapi', type=argparse.FileType('r'))
    parser.add_argument('interfaces_header', metavar='/path/to/cateyes-interfaces.h', type=argparse.FileType('r'))
