...
class DeviceManager(object):
    ...
    def get_device(self, device_id):
        ...
        raise _cateyes.InvalidArgumentError("unable to find device with id %s" % device_id)
...
class Device(object):
    ...
    def get_process(self, process_name):
        ...
        if len(matching) == 1:
            ...
            raise _cateyes.ProcessNotFoundError("ambiguous name; it matches: %s" % ", ".join(["%s (pid: %d)" % (process.name, process.pid) for process in matching]))
        else:
            raise _cateyes.ProcessNotFoundError("unable to find process with name '%s'" % process_name)
...
class Script(object):
    ...
    def _rpc_request(self, *args):
        ...
        message = ['cateyes:rpc', request_id]
        message.extend(args)
        try:
            self.post(message)
    ...
    def _on_destroyed(self):
        while True:
            ...
            next_pending(None, _cateyes.InvalidOperationError('script is destroyed'))
    ...
    def _on_message(self, raw_message, data):
        ...
        elif mtype == 'send' and isinstance(payload, list) and payload[0] == 'cateyes:rpc':
