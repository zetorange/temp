...
typedef struct _CateyesJitRequest CateyesJitRequest;

struct _CateyesJitRequest
{
  union __RequestUnion__cateyes_jit_subsystem body;
...
static gpointer cateyes_jit_server_process_messages (gpointer data);
static gboolean cateyes_jit_server_get_region (mach_vm_address_t * jit_base, mach_vm_size_t * jit_size);

void
_cateyes_server_ios_configure (void)
{
  ...
  kr = bootstrap_check_in (bootstrap_port, "com.apple.uikit.viewservice.cateyes", &listening_port);
  if (kr != KERN_SUCCESS)
    goto checkin_error;

  g_thread_unref (g_thread_new ("jit-server", cateyes_jit_server_process_messages, GSIZE_TO_POINTER (listening_port)));
...
static gpointer
cateyes_jit_server_process_messages (gpointer data)
{
  mach_port_t listening_port = GPOINTER_TO_SIZE (data);
  CateyesJitRequest request;
  union __ReplyUnion__cateyes_jit_subsystem reply;
  ...
  while (TRUE)
  {
    ...
    handled = cateyes_jit_server (header_in, header_out);
    if (handled)
...
kern_return_t
cateyes_jit_alloc (mach_port_t server, vm_task_entry_t task, mach_vm_address_t * address, mach_vm_size_t size, int flags)
{
  ...
  if (!cateyes_jit_server_get_region (&jit_base, &jit_size))
    return KERN_FAILURE;
...
static gboolean
cateyes_jit_server_get_region (mach_vm_address_t * jit_base, mach_vm_size_t * jit_size)
{
