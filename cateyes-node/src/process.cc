...
Process::Process(CateyesProcess* handle, Runtime* runtime)
...
NAN_METHOD(Process::New) {
  ...
  auto handle = static_cast<CateyesProcess*>(
...
NAN_PROPERTY_GETTER(Process::GetPid) {
  auto handle = ObjectWrap::Unwrap<Process>(
      info.Holder())->GetHandle<CateyesProcess>();

  info.GetReturnValue().Set(Nan::New<v8::Integer>(
      cateyes_process_get_pid(handle)));
}
...
NAN_PROPERTY_GETTER(Process::GetName) {
  ...
  info.GetReturnValue().Set(
      Nan::New(cateyes_process_get_name(handle)).ToLocalChecked());
}

NAN_PROPERTY_GETTER(Process::GetSmallIcon) {
  auto wrapper = ObjectWrap::Unwrap<Process>(info.Holder());
  auto handle = wrapper->GetHandle<CateyesProcess>();

  info.GetReturnValue().Set(
      Icon::New(cateyes_process_get_small_icon(handle), wrapper->runtime_));
}
...
NAN_PROPERTY_GETTER(Process::GetLargeIcon) {
  ...
  info.GetReturnValue().Set(
      Icon::New(cateyes_process_get_large_icon(handle), wrapper->runtime_));
}
