#include "cateyes-helper-backend.h"
...
typedef struct _CateyesHelperContext CateyesHelperContext;
typedef struct _CateyesSpawnInstance CateyesSpawnInstance;
typedef struct _CateyesSpawnInstanceDyldData CateyesSpawnInstanceDyldData;
typedef guint CateyesBreakpointPhase;
typedef struct _CateyesInjectInstance CateyesInjectInstance;
typedef struct _CateyesInjectPayloadLayout CateyesInjectPayloadLayout;
typedef struct _CateyesAgentDetails CateyesAgentDetails;
typedef struct _CateyesAgentContext CateyesAgentContext;
typedef struct _CateyesAgentEmitContext CateyesAgentEmitContext;

typedef struct _CateyesExceptionPortSet CateyesExceptionPortSet;
typedef union _CateyesDebugState CateyesDebugState;

struct _CateyesHelperContext
{
  dispatch_queue_t dispatch_queue;
};

struct _CateyesExceptionPortSet
{
...
struct _CateyesSpawnInstance
{
  CateyesDarwinHelperBackend * backend;
  guint pid;
  GumCpuType cpu_type;
  mach_port_t thread;
  CateyesDebugState previous_debug_state;
  CateyesDebugState dlerror_clear_debug_state;
  CateyesDebugState ret_gadget_debug_state;
  CateyesDebugState breakpoint_debug_state;

  mach_port_t server_port;
  dispatch_source_t server_recv_source;
  CateyesExceptionPortSet previous_ports;

  __Request__exception_raise_state_identity_t pending_request;

  CateyesBreakpointPhase breakpoint_phase;
...
enum _CateyesBreakpointPhase
{
  CATEYES_BREAKPOINT_DETECT_FLAVOR,
  CATEYES_BREAKPOINT_SET_HELPERS,
  CATEYES_BREAKPOINT_DLOPEN_LIBC,
  CATEYES_BREAKPOINT_SKIP_CLEAR,
  CATEYES_BREAKPOINT_JUST_RETURN,
  CATEYES_BREAKPOINT_CLEANUP,
  CATEYES_BREAKPOINT_DONE
};

struct _CateyesSpawnInstanceDyldData
{
  const char libc[32];
  guint8 helpers[32];
};

struct _CateyesInjectInstance
{
  guint id;
  ...
  CateyesAgentContext * agent_context;
...
struct _CateyesInjectPayloadLayout
{
...
struct _CateyesAgentContext
{
  /* State */
  CateyesUnloadPolicy unload_policy;
  mach_port_t task;
...
struct _CateyesAgentEmitContext
{
  guint8 * code;
#ifdef HAVE_I386
  GumX86Writer cw;
...
static CateyesSpawnInstance * cateyes_spawn_instance_new (CateyesDarwinHelperBackend * backend);
static void cateyes_spawn_instance_free (CateyesSpawnInstance * instance);
static void cateyes_spawn_instance_resume (CateyesSpawnInstance * self);

static void cateyes_spawn_instance_receive_breakpoint_request (CateyesSpawnInstance * self);
static void cateyes_spawn_instance_send_breakpoint_response (CateyesSpawnInstance * self);
static void cateyes_spawn_instance_create_dyld_data (CateyesSpawnInstance * self);
static void cateyes_spawn_instance_destroy_dyld_data (CateyesSpawnInstance * self);
static void cateyes_spawn_instance_unset_helpers (CateyesSpawnInstance * self);
static gboolean cateyes_spawn_instance_is_libc_initialized (CateyesSpawnInstance * self);
static void cateyes_spawn_instance_on_server_recv (void * context);
static void cateyes_spawn_instance_call_set_helpers (CateyesSpawnInstance * self, GumDarwinUnifiedThreadState state, mach_vm_address_t helpers);
static void cateyes_spawn_instance_call_dlopen (CateyesSpawnInstance * self, GumDarwinUnifiedThreadState state, mach_vm_address_t lib_name, int mode);

static void cateyes_make_pipe (int fds[2]);

static CateyesInjectInstance * cateyes_inject_instance_new (CateyesDarwinHelperBackend * backend, guint id);
static CateyesInjectInstance * cateyes_inject_instance_clone (const CateyesInjectInstance * instance, guint id);
static void cateyes_inject_instance_free (CateyesInjectInstance * instance);
static gboolean cateyes_inject_instance_task_did_not_exec (CateyesInjectInstance * instance);
static gboolean cateyes_inject_instance_is_resident (CateyesInjectInstance * instance);

static gboolean cateyes_inject_instance_start_thread (CateyesInjectInstance * self, GError ** error);
static void cateyes_inject_instance_on_mach_thread_dead (void * context);
static void cateyes_inject_instance_join_posix_thread (CateyesInjectInstance * self, mach_port_t posix_thread);
static void cateyes_inject_instance_on_posix_thread_dead (void * context);

static gboolean cateyes_agent_context_init (CateyesAgentContext * self, const CateyesAgentDetails * details, const CateyesInjectPayloadLayout * layout,
...
static gboolean cateyes_agent_context_init_functions (CateyesAgentContext * self, GumDarwinModuleResolver * resolver, GumDarwinMapper * mapper,
    GError ** error);

static void cateyes_agent_context_emit_mach_stub_code (CateyesAgentContext * self, guint8 * code, GumCpuType cpu_type, GumDarwinMapper * mapper);
static void cateyes_agent_context_emit_pthread_stub_code (CateyesAgentContext * self, guint8 * code, GumCpuType cpu_type, GumDarwinMapper * mapper);

static kern_return_t cateyes_get_debug_state (mach_port_t thread, gpointer state, GumCpuType cpu_type);
static kern_return_t cateyes_set_debug_state (mach_port_t thread, gconstpointer state, GumCpuType cpu_type);
static void cateyes_set_nth_hardware_breakpoint (gpointer state, guint n, GumAddress break_at, GumCpuType cpu_type);

static GumAddress cateyes_find_run_initializers_call (mach_port_t task, GumCpuType cpu_type, GumAddress start);
static GumAddress cateyes_find_function_end (mach_port_t task, GumCpuType cpu_type, GumAddress start, gsize max_size);
static csh cateyes_create_capstone (GumCpuType cpu_type, GumAddress start);

static void cateyes_mapper_library_blob_deallocate (CateyesMappedLibraryBlob * self);

extern int fileport_makeport (int fd, mach_port_t * port);

void
cateyes_darwin_helper_backend_make_pipe_endpoints (guint local_task, guint remote_pid, guint remote_task, CateyesPipeEndpoints * result, GError ** error)
{
  ...
  cateyes_pipe_endpoints_init (result, local_address, remote_address);
...
guint
cateyes_darwin_helper_backend_task_for_pid_fallback (guint pid, GError ** error)
{
...
invalid_pid:
  {
    g_set_error (error,
        CATEYES_ERROR,
        CATEYES_ERROR_PROCESS_NOT_FOUND,
...
permission_denied:
  {
    g_set_error (error,
        CATEYES_ERROR,
        CATEYES_ERROR_PERMISSION_DENIED,
...
void
cateyes_darwin_helper_backend_deallocate_port (guint port)
{
  mach_port_deallocate (mach_task_self (), port);
}

gboolean
cateyes_darwin_helper_backend_is_mmap_available (void)
{
#ifdef HAVE_MAPPER
  return TRUE;
#else
...
void
cateyes_darwin_helper_backend_mmap (guint task, GBytes * blob, CateyesMappedLibraryBlob * result, GError ** error)
{
  gconstpointer data;
  ...
  cateyes_mapped_library_blob_init (result, mapped_address, size, aligned_size);
...
void
_cateyes_darwin_helper_backend_create_context (CateyesDarwinHelperBackend * self)
{
  CateyesHelperContext * ctx;

  ctx = g_slice_new (CateyesHelperContext);
  ctx->dispatch_queue = dispatch_queue_create ("re.cateyes.helper.queue", DISPATCH_QUEUE_SERIAL);

  self->context = ctx;
}

void
_cateyes_darwin_helper_backend_destroy_context (CateyesDarwinHelperBackend * self)
{
  CateyesHelperContext * ctx = self->context;

  dispatch_release (ctx->dispatch_queue);

  g_slice_free (CateyesHelperContext, ctx);
}

guint
_cateyes_darwin_helper_backend_spawn (CateyesDarwinHelperBackend * self, const gchar * path,
    CateyesHostSpawnOptions * options, CateyesStdioPipes ** pipes, GError ** error)
{
  pid_t pid;
  CateyesSpawnInstance * instance;
  gchar ** argv, ** envp;
  ...
  instance = cateyes_spawn_instance_new (self);

  argv = cateyes_host_spawn_options_compute_argv (options, path, NULL);
  envp = cateyes_host_spawn_options_compute_envp (options, NULL);

  aux_options = cateyes_host_spawn_options_load_aux (options);
  ...
  stdio = cateyes_host_spawn_options_get_stdio (options);
  switch (stdio)
  {
    case CATEYES_STDIO_INHERIT:
      *pipes = NULL;
    ...
    case CATEYES_STDIO_PIPE:
      cateyes_make_pipe (stdin_pipe);
      cateyes_make_pipe (stdout_pipe);
      cateyes_make_pipe (stderr_pipe);

      *pipes = cateyes_stdio_pipes_new (stdin_pipe[1], stdout_pipe[0], stderr_pipe[0]);
  ...
  cwd = cateyes_host_spawn_options_get_cwd (options);
  if (strlen (cwd) > 0)
  {
...
spawn_failed:
  {
    if (spawn_errno == EAGAIN)
    {
      g_set_error (error,
          CATEYES_ERROR,
          CATEYES_ERROR_EXECUTABLE_NOT_SUPPORTED,
...
failure:
  {
    ...
    cateyes_spawn_instance_free (instance);
...
static void cateyes_darwin_helper_backend_launch_using_fbs (NSString * identifier, NSURL * url,
    CateyesHostSpawnOptions * spawn_options, GVariantDict * aux_options,
    CateyesDarwinHelperBackendLaunchCompletionHandler on_complete, void * on_complete_target);
static void cateyes_darwin_helper_backend_launch_using_sbs (NSString * identifier, NSURL * url,
    CateyesHostSpawnOptions * spawn_options, GVariantDict * aux_options,
    CateyesDarwinHelperBackendLaunchCompletionHandler on_complete, void * on_complete_target);

static void cateyes_kill_application (NSString * identifier);

static NSArray * cateyes_argv_to_arguments_array (gchar * const * argv, gint argv_length);
static NSDictionary * cateyes_envp_to_environment_dictionary (gchar * const * envp, gint envp_length);

static void cateyes_configure_terminal_attributes (gint fd);

void
_cateyes_darwin_helper_backend_launch (const gchar * identifier, CateyesHostSpawnOptions * options,
    CateyesDarwinHelperBackendLaunchCompletionHandler on_complete, void * on_complete_target)
{
  ...
  if (_cateyes_get_springboard_api ()->fbs != NULL)
  {
    cateyes_darwin_helper_backend_launch_using_fbs (identifier_value, url_value, options, aux_options, on_complete,
        on_complete_target);
  }
  else
  {
    cateyes_darwin_helper_backend_launch_using_sbs (identifier_value, url_value, options, aux_options, on_complete,
        on_complete_target);
  }
...
static void
cateyes_darwin_helper_backend_launch_using_fbs (NSString * identifier, NSURL * url, CateyesHostSpawnOptions * spawn_options,
    GVariantDict * aux_options, CateyesDarwinHelperBackendLaunchCompletionHandler on_complete, void * on_complete_target)
{
  CateyesSpringboardApi * api;
  NSMutableDictionary * debug_options, * open_options;
  CateyesStdioPipes * pipes = NULL;
  GError * error = NULL;
  gchar * aslr = NULL;
  ...
  if (cateyes_host_spawn_options_get_has_argv (spawn_options))
  {
    gchar ** argv;
    gint argv_length;

    argv = cateyes_host_spawn_options_get_argv (spawn_options, &argv_length);

    [debug_options setObject:cateyes_argv_to_arguments_array (argv, argv_length)
                      forKey:api->FBSDebugOptionKeyArguments];
  }

  if (cateyes_host_spawn_options_get_has_envp (spawn_options))
    goto envp_not_supported;

  if (cateyes_host_spawn_options_get_has_env (spawn_options))
  {
    gchar ** env;
    gint env_length;

    env = cateyes_host_spawn_options_get_env (spawn_options, &env_length);

    [debug_options setObject:cateyes_envp_to_environment_dictionary (env, env_length)
                      forKey:api->FBSDebugOptionKeyEnvironment];
  }

  if (strlen (cateyes_host_spawn_options_get_cwd (spawn_options)) > 0)
    goto cwd_not_supported;

  if (cateyes_host_spawn_options_get_stdio (spawn_options) == CATEYES_STDIO_PIPE)
  {
    ...
    pipes = cateyes_stdio_pipes_new (-1, stdout_master, stderr_master);

    cateyes_configure_terminal_attributes (stdout_master);
    cateyes_configure_terminal_attributes (stderr_master);

    cateyes_stdio_pipes_retain (pipes, stdout_slave);
    cateyes_stdio_pipes_retain (pipes, stderr_slave);
  ...
  result_callback = ^(NSError * error)
  {
    ...
    on_complete (pipes, cateyes_error, on_complete_target);
  };

  dispatch_async (dispatch_get_global_queue (DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^
  {
    cateyes_kill_application (identifier);
...
static void
cateyes_darwin_helper_backend_launch_using_sbs (NSString * identifier, NSURL * url, CateyesHostSpawnOptions * spawn_options,
    GVariantDict * aux_options, CateyesDarwinHelperBackendLaunchCompletionHandler on_complete, void * on_complete_target)
{
  CateyesSpringboardApi * api;
  ...
  if (cateyes_host_spawn_options_get_stdio (spawn_options) != CATEYES_STDIO_INHERIT)
...
void
_cateyes_darwin_helper_backend_kill_process (guint pid)
{
  NSAutoreleasePool * pool;
  NSString * identifier;
  ...
  identifier = _cateyes_get_springboard_api ()->SBSCopyDisplayIdentifierForProcessID (pid);
  if (identifier != nil)
  {
    cateyes_kill_application (identifier);
...
void
_cateyes_darwin_helper_backend_kill_application (const gchar * identifier)
{
  NSAutoreleasePool * pool;

  pool = [[NSAutoreleasePool alloc] init];

  cateyes_kill_application ([NSString stringWithUTF8String:identifier]);

  [pool release];
}

static void
cateyes_kill_application (NSString * identifier)
{
  CateyesSpringboardApi * api;
  GTimer * timer;
  ...
  if (api->FBSSystemService != nil)
  {
    ...
    [service terminateApplication:identifier
                        ...
                        andReport:NO
                  withDescription:@"killed from Cateyes"];
...
static NSArray *
cateyes_argv_to_arguments_array (gchar * const * argv, gint argv_length)
{
...
static NSDictionary *
cateyes_envp_to_environment_dictionary (gchar * const * envp, gint envp_length)
{
...
static void
cateyes_configure_terminal_attributes (gint fd)
{
  struct termios tios;
...
gboolean
_cateyes_darwin_helper_backend_is_suspended (CateyesDarwinHelperBackend * self, guint task, GError ** error)
{
...
void
_cateyes_darwin_helper_backend_resume_process (CateyesDarwinHelperBackend * self, guint task, GError ** error)
{
  mach_task_basic_info_data_t info;
  mach_msg_type_number_t info_count = MACH_TASK_BASIC_INFO;
...
process_not_suspended:
  {
    g_set_error (error,
        CATEYES_ERROR,
        CATEYES_ERROR_INVALID_OPERATION,
...
void
_cateyes_darwin_helper_backend_resume_process_fast (CateyesDarwinHelperBackend * self, guint task, GError ** error)
{
  kern_return_t kr;
...
void *
_cateyes_darwin_helper_backend_create_spawn_instance (CateyesDarwinHelperBackend * self, guint pid)
{
  CateyesSpawnInstance * instance;
...
void
_cateyes_darwin_helper_backend_prepare_spawn_instance_for_injection (CateyesDarwinHelperBackend * self, void * opaque_instance, guint task, GError ** error)
{
  CateyesSpawnInstance * instance = opaque_instance;
  CateyesHelperContext * ctx = self->context;
  const gchar * failed_operation;
  kern_return_t kr;
   ...
   * initialized, so we won't be able to inject cateyes-agent at this point.
   *
   ...
   *   sufficiently initialized to load cateyes-agent, but still early enough so the app's
  ...
  if (launch_with_closure_address != 0)
  {
    modern_entry_address = cateyes_find_run_initializers_call (task, instance->cpu_type, launch_with_closure_address);
  }
  ...
  instance->ret_gadget = cateyes_find_function_end (task, instance->cpu_type, instance->register_helpers_address, 128);
  ...
  cateyes_spawn_instance_create_dyld_data (instance);

  kr = cateyes_get_debug_state (child_thread, &instance->previous_debug_state, instance->cpu_type);
  CHECK_MACH_RESULT (kr, ==, KERN_SUCCESS, "cateyes_get_debug_state");

  memcpy (&instance->breakpoint_debug_state, &instance->previous_debug_state, sizeof (instance->breakpoint_debug_state));
  cateyes_set_nth_hardware_breakpoint (&instance->breakpoint_debug_state, 0, legacy_entry_address, instance->cpu_type);
  if (modern_entry_address != 0)
  {
    cateyes_set_nth_hardware_breakpoint (&instance->breakpoint_debug_state, 1, modern_entry_address, instance->cpu_type);
  }
  ...
  if (instance->dlerror_clear_address != 0)
  {
    ...
    cateyes_set_nth_hardware_breakpoint (&instance->dlerror_clear_debug_state, 0, instance->dlerror_clear_address, instance->cpu_type);
  }

  memcpy (&instance->ret_gadget_debug_state, &instance->previous_debug_state, sizeof (instance->previous_debug_state));
  cateyes_set_nth_hardware_breakpoint (&instance->ret_gadget_debug_state, 0, instance->ret_gadget & ~1, instance->cpu_type);

  kr = cateyes_set_debug_state (child_thread, &instance->breakpoint_debug_state, instance->cpu_type);
  CHECK_MACH_RESULT (kr, ==, KERN_SUCCESS, "cateyes_set_debug_state");
  ...
  dispatch_source_set_event_handler_f (source, cateyes_spawn_instance_on_server_recv);
...
void
_cateyes_darwin_helper_backend_resume_spawn_instance (CateyesDarwinHelperBackend * self, void * instance)
{
  cateyes_spawn_instance_resume (instance);
}

void
_cateyes_darwin_helper_backend_free_spawn_instance (CateyesDarwinHelperBackend * self, void * instance)
{
  cateyes_spawn_instance_free (instance);
}

guint
_cateyes_darwin_helper_backend_inject_into_task (CateyesDarwinHelperBackend * self, guint pid, guint task, const gchar * path_or_name, CateyesMappedLibraryBlob * blob,
    const gchar * entrypoint, const gchar * data, GError ** error)
{
  guint result = 0;
  mach_port_t self_task;
  CateyesInjectInstance * instance;
  GumDarwinModuleResolver * resolver = NULL;
  GumDarwinMapper * mapper = NULL;
  CateyesAgentDetails details = { 0, };
  guint page_size;
  CateyesInjectPayloadLayout layout;
  kern_return_t kr;
  const gchar * failed_operation;
  ...
  CateyesAgentContext agent_ctx;

  self_task = mach_task_self ();

  instance = cateyes_inject_instance_new (self, self->next_id++);
  ...
  if (blob != NULL)
  {
    mapper = gum_darwin_mapper_new_take_blob (path_or_name,
        g_bytes_new_with_free_func (GSIZE_TO_POINTER (blob->_address), blob->_size,
            (GDestroyNotify) cateyes_mapper_library_blob_deallocate, cateyes_mapped_library_blob_dup (blob)),
        resolver);
  }
  else
  {
...
#else
  (void) cateyes_mapper_library_blob_deallocate;
#endif
  ...
  instance->agent_context = (CateyesAgentContext *) agent_context_address;
  ...
  if (!cateyes_agent_context_init (&agent_ctx, &details, &layout, payload_address, instance->payload_size, resolver, mapper, error))
    goto failure;

  cateyes_agent_context_emit_mach_stub_code (&agent_ctx, mach_stub_code, details.cpu_type, mapper);

  cateyes_agent_context_emit_pthread_stub_code (&agent_ctx, pthread_stub_code, details.cpu_type, mapper);
  ...
  else
  {
    ...
    state32->__cpsr = CATEYES_PSR_THUMB;
  ...
  if (!cateyes_inject_instance_start_thread (instance, error))
    goto failure;
...
failure:
  {
    cateyes_inject_instance_free (instance);
    goto beach;
  }
beach:
  {
...
guint
_cateyes_darwin_helper_backend_demonitor_and_clone_injectee_state (CateyesDarwinHelperBackend * self, void * raw_instance)
{
  CateyesInjectInstance * instance = raw_instance;
  CateyesInjectInstance * clone;
  ...
  clone = cateyes_inject_instance_clone (instance, self->next_id++);
...
void
_cateyes_darwin_helper_backend_recreate_injectee_thread (CateyesDarwinHelperBackend * self, void * raw_instance, guint pid, guint task, GError ** error)
{
  CateyesInjectInstance * instance = raw_instance;
  CateyesAgentContext * agent_context = instance->agent_context;
  mach_port_t self_task;
  gboolean is_uninitialized_clone;
  const gchar * failed_operation;
  kern_return_t kr;

  agent_context->unload_policy = CATEYES_UNLOAD_POLICY_IMMEDIATE;
...
failure:
  {
    _cateyes_darwin_helper_backend_destroy_inject_instance (self, instance->id);
    goto beach;
  }
beach:
  {
...
static gboolean
cateyes_inject_instance_start_thread (CateyesInjectInstance * self, GError ** error)
{
  gboolean success = FALSE;
  kern_return_t kr;
  const gchar * failed_operation;
  CateyesHelperContext * ctx = self->backend->context;
  dispatch_source_t source;
  ...
  dispatch_source_set_event_handler_f (source, cateyes_inject_instance_on_mach_thread_dead);
  dispatch_resume (source);
...
static void
cateyes_inject_instance_on_mach_thread_dead (void * context)
{
  CateyesInjectInstance * self = context;
  ...
  _cateyes_darwin_helper_backend_on_mach_thread_dead (self->backend, self->id, GSIZE_TO_POINTER (posix_thread_right_in_local_task));
}

void
_cateyes_darwin_helper_backend_join_inject_instance_posix_thread (CateyesDarwinHelperBackend * self, void * instance, void * posix_thread)
{
  cateyes_inject_instance_join_posix_thread (instance, GPOINTER_TO_SIZE (posix_thread));
}

static void
cateyes_inject_instance_join_posix_thread (CateyesInjectInstance * self, mach_port_t posix_thread)
{
  CateyesHelperContext * ctx = self->backend->context;
  mach_port_t self_task;
  dispatch_source_t source;
  ...
  self->thread_monitor_source = source;
  dispatch_set_context (source, self);
  dispatch_source_set_event_handler_f (source, cateyes_inject_instance_on_posix_thread_dead);
  dispatch_resume (source);
}

static void
cateyes_inject_instance_on_posix_thread_dead (void * context)
{
  CateyesInjectInstance * self = context;

  _cateyes_darwin_helper_backend_on_posix_thread_dead (self->backend, self->id);
}

gboolean
_cateyes_darwin_helper_backend_is_instance_resident (CateyesDarwinHelperBackend * self, void * instance)
{
  return cateyes_inject_instance_is_resident (instance);
}

void
_cateyes_darwin_helper_backend_free_inject_instance (CateyesDarwinHelperBackend * self, void * instance)
{
  cateyes_inject_instance_free (instance);
}

static CateyesSpawnInstance *
cateyes_spawn_instance_new (CateyesDarwinHelperBackend * backend)
{
  CateyesSpawnInstance * instance;

  instance = g_slice_new0 (CateyesSpawnInstance);
  ...
  instance->breakpoint_phase = CATEYES_BREAKPOINT_DETECT_FLAVOR;

  return instance;
}

static void
cateyes_spawn_instance_free (CateyesSpawnInstance * instance)
{
  task_t self_task;
  CateyesExceptionPortSet * previous_ports;
  ...
  g_slice_free (CateyesSpawnInstance, instance);
}

static void
cateyes_spawn_instance_resume (CateyesSpawnInstance * self)
{
  if (self->breakpoint_phase != CATEYES_BREAKPOINT_DONE)
  {
    guint task;
    GError * error = NULL;

    task = cateyes_darwin_helper_backend_steal_task_for_remote_pid (self->backend, self->pid, &error);
    if (error == NULL)
    {
      _cateyes_darwin_helper_backend_resume_process (self->backend, task, &error);
  ...
  cateyes_spawn_instance_send_breakpoint_response (self);
}

static void
cateyes_spawn_instance_receive_breakpoint_request (CateyesSpawnInstance * self)
{
...
static void
cateyes_spawn_instance_send_breakpoint_response (CateyesSpawnInstance * self)
{
  __Request__exception_raise_state_identity_t * request = &self->pending_request;
...
static void
cateyes_spawn_instance_create_dyld_data (CateyesSpawnInstance * self)
{
  kern_return_t kr;
  gboolean write_succeeded;
  CateyesSpawnInstanceDyldData data = { "/usr/lib/libSystem.B.dylib", { 0, } };
  ...
  self->fake_helpers = self->dyld_data + offsetof (CateyesSpawnInstanceDyldData, helpers);
  self->lib_name = self->dyld_data + offsetof (CateyesSpawnInstanceDyldData, libc);
}

static void
cateyes_spawn_instance_destroy_dyld_data (CateyesSpawnInstance * self)
{
  kern_return_t kr;
  ...
  kr = vm_deallocate (self->task, (vm_address_t) self->dyld_data, sizeof (CateyesSpawnInstanceDyldData));
  g_assert_cmpint (kr, ==, KERN_SUCCESS);
...
static void
cateyes_spawn_instance_unset_helpers (CateyesSpawnInstance * self)
{
  gboolean write_succeeded;
...
static void
cateyes_spawn_instance_set_libc_initialized (CateyesSpawnInstance * self)
{
...
static gboolean
cateyes_spawn_instance_is_libc_initialized (CateyesSpawnInstance * self)
{
...
static void
cateyes_spawn_instance_on_server_recv (void * context)
{
  CateyesSpawnInstance * self = context;
  kern_return_t kr;
  thread_state_flavor_t state_flavor = GUM_DARWIN_THREAD_STATE_FLAVOR;
  mach_msg_type_number_t state_count = GUM_DARWIN_THREAD_STATE_COUNT;
  GumDarwinUnifiedThreadState state;

  cateyes_spawn_instance_receive_breakpoint_request (self);

  if (self->breakpoint_phase == CATEYES_BREAKPOINT_DETECT_FLAVOR)
  {
    GumAddress pc;
    ...
    if (pc == self->modern_entry_address)
      self->breakpoint_phase = CATEYES_BREAKPOINT_CLEANUP;
    else
      self->breakpoint_phase = self->need_helpers ? CATEYES_BREAKPOINT_SET_HELPERS : CATEYES_BREAKPOINT_DLOPEN_LIBC;
  }

  switch (self->breakpoint_phase)
  {
    case CATEYES_BREAKPOINT_SET_HELPERS:
      cateyes_spawn_instance_call_set_helpers (self, state, self->fake_helpers);

      self->breakpoint_phase = CATEYES_BREAKPOINT_DLOPEN_LIBC;
      cateyes_spawn_instance_send_breakpoint_response (self);

      break;

    case CATEYES_BREAKPOINT_DLOPEN_LIBC:
      if (cateyes_spawn_instance_is_libc_initialized (self))
        cateyes_spawn_instance_unset_helpers (self);

      memcpy (&state, &self->previous_thread_state, sizeof (state));

      cateyes_spawn_instance_call_dlopen (self, state, self->lib_name, RTLD_GLOBAL | RTLD_LAZY);

      if (self->dlerror_clear_address != 0)
      {
        cateyes_set_debug_state (self->thread, &self->dlerror_clear_debug_state, self->cpu_type);
        self->breakpoint_phase = CATEYES_BREAKPOINT_SKIP_CLEAR;
      }
      else
      {
        cateyes_set_debug_state (self->thread, &self->breakpoint_debug_state, self->cpu_type);
        self->breakpoint_phase = CATEYES_BREAKPOINT_CLEANUP;
      }
    ...
    case CATEYES_BREAKPOINT_SKIP_CLEAR:
    case CATEYES_BREAKPOINT_JUST_RETURN:
      kr = thread_get_state (self->thread, state_flavor, (thread_state_t) &state, &state_count);
      g_assert_cmpint (kr, ==, KERN_SUCCESS);
      ...
      if (self->breakpoint_phase == CATEYES_BREAKPOINT_SKIP_CLEAR && self->cpu_type == GUM_CPU_ARM)
      {
        cateyes_set_debug_state (self->thread, &self->ret_gadget_debug_state, self->cpu_type);
        self->breakpoint_phase = CATEYES_BREAKPOINT_JUST_RETURN;
      }
      else
      {
        cateyes_set_debug_state (self->thread, &self->breakpoint_debug_state, self->cpu_type);
    ...
    case CATEYES_BREAKPOINT_CLEANUP:
    {
      task_t self_task;
      CateyesExceptionPortSet * previous_ports;
      mach_msg_type_number_t port_index;
      ...
      cateyes_spawn_instance_destroy_dyld_data (self);

      cateyes_set_debug_state (self->thread, &self->previous_debug_state, self->cpu_type);

      cateyes_spawn_instance_set_libc_initialized (self);

      self->breakpoint_phase = CATEYES_BREAKPOINT_DONE;

      _cateyes_darwin_helper_backend_on_spawn_instance_ready (self->backend, self->pid);
...
static void
cateyes_spawn_instance_call_set_helpers (CateyesSpawnInstance * self, GumDarwinUnifiedThreadState state, mach_vm_address_t helpers)
{
  kern_return_t kr;
...
static void
cateyes_spawn_instance_call_dlopen (CateyesSpawnInstance * self, GumDarwinUnifiedThreadState state, mach_vm_address_t lib_name, int mode)
{
  kern_return_t kr;
  GumAddress current_pc;
...
static CateyesInjectInstance *
cateyes_inject_instance_new (CateyesDarwinHelperBackend * backend, guint id)
{
  CateyesInjectInstance * instance;

  instance = g_slice_new (CateyesInjectInstance);
  instance->id = id;
...
static CateyesInjectInstance *
cateyes_inject_instance_clone (const CateyesInjectInstance * instance, guint id)
{
  CateyesInjectInstance * clone;
  mach_port_t self_task;
  mach_vm_address_t agent_context_address = 0;
  ...
  clone = g_slice_dup (CateyesInjectInstance, instance);
  clone->id = id;
  ...
  clone->agent_context = (CateyesAgentContext *) agent_context_address;
...
static void
cateyes_inject_instance_free (CateyesInjectInstance * instance)
{
  CateyesAgentContext * agent_context = instance->agent_context;
  task_t self_task;
  ...
  can_deallocate_payload = !(agent_context != NULL && agent_context->unload_policy != CATEYES_UNLOAD_POLICY_IMMEDIATE && instance->is_mapped);
  if (instance->payload_address != 0 &&
      can_deallocate_payload &&
      cateyes_inject_instance_task_did_not_exec (instance))
  {
  ...
  g_slice_free (CateyesInjectInstance, instance);
}

static gboolean
cateyes_inject_instance_task_did_not_exec (CateyesInjectInstance * instance)
{
...
static gboolean
cateyes_inject_instance_is_resident (CateyesInjectInstance * instance)
{
  return instance->agent_context->unload_policy != CATEYES_UNLOAD_POLICY_IMMEDIATE;
}

static gboolean
cateyes_agent_context_init (CateyesAgentContext * self, const CateyesAgentDetails * details, const CateyesInjectPayloadLayout * layout,
    mach_vm_address_t payload_base, mach_vm_size_t payload_size, GumDarwinModuleResolver * resolver, GumDarwinMapper * mapper, GError ** error)
{
  bzero (self, sizeof (CateyesAgentContext));

  self->unload_policy = CATEYES_UNLOAD_POLICY_IMMEDIATE;
  ...
  if (!cateyes_agent_context_init_functions (self, resolver, mapper, error))
    return FALSE;
  ...
  self->message_that_never_arrives = payload_base + layout->data_offset +
      G_STRUCT_OFFSET (CateyesAgentContext, message_that_never_arrives_storage);
  ...
  self->dylib_path = payload_base + layout->data_offset +
      G_STRUCT_OFFSET (CateyesAgentContext, dylib_path_storage);
  ...
  self->entrypoint_name = payload_base + layout->data_offset +
      G_STRUCT_OFFSET (CateyesAgentContext, entrypoint_name_storage);
  strcpy (self->entrypoint_name_storage, details->entrypoint_name);

  self->entrypoint_data = payload_base + layout->data_offset +
      G_STRUCT_OFFSET (CateyesAgentContext, entrypoint_data_storage);
  ...
  self->mapped_range = (mapper != NULL)
      ? payload_base + layout->data_offset + G_STRUCT_OFFSET (CateyesAgentContext, mapped_range_storage)
      : 0;
...
#define CATEYES_AGENT_CONTEXT_RESOLVE(field) \
  G_STMT_START \
  { \
    CATEYES_AGENT_CONTEXT_TRY_RESOLVE (field); \
    if (self->field##_impl == 0) \
      goto missing_symbol; \
  } \
  G_STMT_END
#define CATEYES_AGENT_CONTEXT_TRY_RESOLVE(field) \
  self->field##_impl = gum_darwin_module_resolver_find_export_address (resolver, module, G_STRINGIFY (field))

static gboolean
cateyes_agent_context_init_functions (CateyesAgentContext * self, GumDarwinModuleResolver * resolver, GumDarwinMapper * mapper, GError ** error)
{
  ...
  if (module == NULL)
    goto no_libc;
  CATEYES_AGENT_CONTEXT_RESOLVE (mach_task_self);
  CATEYES_AGENT_CONTEXT_RESOLVE (mach_thread_self);
  CATEYES_AGENT_CONTEXT_RESOLVE (mach_port_allocate);
  CATEYES_AGENT_CONTEXT_RESOLVE (mach_msg_receive);
  CATEYES_AGENT_CONTEXT_RESOLVE (mach_port_destroy);
  CATEYES_AGENT_CONTEXT_RESOLVE (thread_terminate);
  ...
  if (module == NULL)
    goto no_libc;
  CATEYES_AGENT_CONTEXT_TRY_RESOLVE (pthread_create_from_mach_thread);
  if (self->pthread_create_from_mach_thread_impl != 0)
    self->pthread_create_impl = self->pthread_create_from_mach_thread_impl;
  else
    CATEYES_AGENT_CONTEXT_RESOLVE (pthread_create);
  CATEYES_AGENT_CONTEXT_RESOLVE (pthread_detach);
  CATEYES_AGENT_CONTEXT_RESOLVE (pthread_self);
  ...
  if (mapper == NULL)
  {
    ...
    if (module == NULL)
      goto no_libc;
    CATEYES_AGENT_CONTEXT_RESOLVE (dlopen);
    CATEYES_AGENT_CONTEXT_RESOLVE (dlsym);
    CATEYES_AGENT_CONTEXT_RESOLVE (dlclose);
  }
...
static void cateyes_agent_context_emit_mach_stub_body (CateyesAgentContext * self, CateyesAgentEmitContext * ctx);
static void cateyes_agent_context_emit_pthread_stub_body (CateyesAgentContext * self, CateyesAgentEmitContext * ctx);

static void
cateyes_agent_context_emit_mach_stub_code (CateyesAgentContext * self, guint8 * code, GumCpuType cpu_type, GumDarwinMapper * mapper)
{
  CateyesAgentEmitContext ctx;
  ...
  cateyes_agent_context_emit_mach_stub_body (self, &ctx);
...
static void
cateyes_agent_context_emit_pthread_stub_code (CateyesAgentContext * self, guint8 * code, GumCpuType cpu_type, GumDarwinMapper * mapper)
{
  CateyesAgentEmitContext ctx;
  guint locals_size;
  ...
  cateyes_agent_context_emit_pthread_stub_body (self, &ctx);
...
#define EMIT_LOAD(reg, field) \
    gum_x86_writer_put_mov_reg_reg_offset_ptr (&ctx->cw, GUM_REG_##reg, GUM_REG_XBX, G_STRUCT_OFFSET (CateyesAgentContext, field))
#define EMIT_LOAD_ADDRESS_OF(reg, field) \
    gum_x86_writer_put_lea_reg_reg_offset (&ctx->cw, GUM_REG_##reg, GUM_REG_XBX, G_STRUCT_OFFSET (CateyesAgentContext, field))
#define EMIT_STORE(field, reg) \
    gum_x86_writer_put_mov_reg_offset_ptr_reg (&ctx->cw, GUM_REG_XBX, G_STRUCT_OFFSET (CateyesAgentContext, field), GUM_REG_##reg)
#define EMIT_CALL(fun, ...) \
    gum_x86_writer_put_call_reg_offset_ptr_with_aligned_arguments (&ctx->cw, GUM_CALL_CAPI, GUM_REG_XBX, G_STRUCT_OFFSET (CateyesAgentContext, fun), __VA_ARGS__)

static void
cateyes_agent_context_emit_mach_stub_body (CateyesAgentContext * self, CateyesAgentEmitContext * ctx)
{
...
static void
cateyes_agent_context_emit_pthread_stub_body (CateyesAgentContext * self, CateyesAgentEmitContext * ctx)
{
  ...
  gum_x86_writer_put_mov_reg_offset_ptr_reg (&ctx->cw,
      GUM_REG_XBP, injector_state_offset + G_STRUCT_OFFSET (CateyesDarwinInjectorState, mapped_range),
      GUM_REG_XDX);
  ...
  gum_x86_writer_put_cmp_reg_i32 (&ctx->cw, GUM_REG_EAX, CATEYES_UNLOAD_POLICY_IMMEDIATE);
  ...
  EMIT_LOAD (EAX, unload_policy);
  gum_x86_writer_put_cmp_reg_i32 (&ctx->cw, GUM_REG_EAX, CATEYES_UNLOAD_POLICY_DEFERRED);
...
static void cateyes_agent_context_emit_arm_mach_stub_code (CateyesAgentContext * self, guint8 * code, GumDarwinMapper * mapper);
static void cateyes_agent_context_emit_arm_pthread_stub_code (CateyesAgentContext * self, guint8 * code, GumDarwinMapper * mapper);
static void cateyes_agent_context_emit_arm_mach_stub_body (CateyesAgentContext * self, CateyesAgentEmitContext * ctx);
static void cateyes_agent_context_emit_arm_pthread_stub_body (CateyesAgentContext * self, CateyesAgentEmitContext * ctx);
static void cateyes_agent_context_emit_arm_load_reg_with_ctx_value (arm_reg reg, guint field_offset, GumThumbWriter * tw);
static void cateyes_agent_context_emit_arm_store_reg_in_ctx_value (guint field_offset, arm_reg reg, GumThumbWriter * tw);

static void cateyes_agent_context_emit_arm64_mach_stub_code (CateyesAgentContext * self, guint8 * code, GumDarwinMapper * mapper);
static void cateyes_agent_context_emit_arm64_pthread_stub_code (CateyesAgentContext * self, guint8 * code, GumDarwinMapper * mapper);
static void cateyes_agent_context_emit_arm64_mach_stub_body (CateyesAgentContext * self, CateyesAgentEmitContext * ctx);
static void cateyes_agent_context_emit_arm64_pthread_stub_body (CateyesAgentContext * self, CateyesAgentEmitContext * ctx);

static void
cateyes_agent_context_emit_mach_stub_code (CateyesAgentContext * self, guint8 * code, GumCpuType cpu_type, GumDarwinMapper * mapper)
{
  if (cpu_type == GUM_CPU_ARM)
    cateyes_agent_context_emit_arm_mach_stub_code (self, code, mapper);
  else
    cateyes_agent_context_emit_arm64_mach_stub_code (self, code, mapper);
}
...
{
  if (cpu_type == GUM_CPU_ARM)
    cateyes_agent_context_emit_arm_pthread_stub_code (self, code, mapper);
  else
    cateyes_agent_context_emit_arm64_pthread_stub_code (self, code, mapper);
}
...
static void
cateyes_agent_context_emit_arm_mach_stub_code (CateyesAgentContext * self, guint8 * code, GumDarwinMapper * mapper)
{
  CateyesAgentEmitContext ctx;
  ...
  cateyes_agent_context_emit_arm_mach_stub_body (self, &ctx);

  gum_thumb_writer_clear (&ctx.tw);
}

static void
cateyes_agent_context_emit_arm_pthread_stub_code (CateyesAgentContext * self, guint8 * code, GumDarwinMapper * mapper)
{
  CateyesAgentEmitContext ctx;
  ...
  cateyes_agent_context_emit_arm_pthread_stub_body (self, &ctx);
...
#define EMIT_ARM_LOAD(reg, field) \
    cateyes_agent_context_emit_arm_load_reg_with_ctx_value (ARM_REG_##reg, G_STRUCT_OFFSET (CateyesAgentContext, field), &ctx->tw)
#define EMIT_ARM_LOAD_ADDRESS_OF(reg, field) \
    gum_thumb_writer_put_add_reg_reg_imm (&ctx->tw, ARM_REG_##reg, ARM_REG_R7, G_STRUCT_OFFSET (CateyesAgentContext, field))
#define EMIT_ARM_LOAD_U32(reg, val) \
    gum_thumb_writer_put_ldr_reg_u32 (&ctx->tw, ARM_REG_##reg, val)
#define EMIT_ARM_STORE(field, reg) \
    cateyes_agent_context_emit_arm_store_reg_in_ctx_value (G_STRUCT_OFFSET (CateyesAgentContext, field), ARM_REG_##reg, &ctx->tw)
...
static void
cateyes_agent_context_emit_arm_mach_stub_body (CateyesAgentContext * self, CateyesAgentEmitContext * ctx)
{
  const gchar * again = "again";
...
static void
cateyes_agent_context_emit_arm_pthread_stub_body (CateyesAgentContext * self, CateyesAgentEmitContext * ctx)
{
  const gchar * skip_construction = "skip_construction";
  const gchar * skip_dlopen = "skip_dlopen";
  const gchar * skip_destruction = "skip_destruction";
  ...
  gum_thumb_writer_put_cmp_reg_imm (&ctx->tw, ARM_REG_R0, CATEYES_UNLOAD_POLICY_IMMEDIATE);
  ...
  EMIT_ARM_LOAD (R0, unload_policy);
  gum_thumb_writer_put_cmp_reg_imm (&ctx->tw, ARM_REG_R0, CATEYES_UNLOAD_POLICY_DEFERRED);
...
static void
cateyes_agent_context_emit_arm_load_reg_with_ctx_value (arm_reg reg, guint field_offset, GumThumbWriter * tw)
{
...
static void
cateyes_agent_context_emit_arm_store_reg_in_ctx_value (guint field_offset, arm_reg reg, GumThumbWriter * tw)
{
  arm_reg tmp_reg = (reg != ARM_REG_R0) ? ARM_REG_R0 : ARM_REG_R1;
  gum_thumb_writer_put_push_regs (tw, 1, tmp_reg);
  gum_thumb_writer_put_ldr_reg_u32 (tw, tmp_reg, field_offset);
...
static void
cateyes_agent_context_emit_arm64_mach_stub_code (CateyesAgentContext * self, guint8 * code, GumDarwinMapper * mapper)
{
  CateyesAgentEmitContext ctx;
  ...
  cateyes_agent_context_emit_arm64_mach_stub_body (self, &ctx);
...
static void
cateyes_agent_context_emit_arm64_pthread_stub_code (CateyesAgentContext * self, guint8 * code, GumDarwinMapper * mapper)
{
  CateyesAgentEmitContext ctx;
  ...
  cateyes_agent_context_emit_arm64_pthread_stub_body (self, &ctx);
  gum_arm64_writer_put_pop_reg_reg (&ctx.aw, ARM64_REG_X19, ARM64_REG_X20);
  gum_arm64_writer_put_pop_reg_reg (&ctx.aw, ARM64_REG_FP, ARM64_REG_LR);
  gum_arm64_writer_put_ret (&ctx.aw);
  gum_arm64_writer_clear (&ctx.aw);
...
#define EMIT_ARM64_LOAD(reg, field) \
    gum_arm64_writer_put_ldr_reg_reg_offset (&ctx->aw, ARM64_REG_##reg, ARM64_REG_X20, G_STRUCT_OFFSET (CateyesAgentContext, field))
#define EMIT_ARM64_LOAD_ADDRESS_OF(reg, field) \
    gum_arm64_writer_put_add_reg_reg_imm (&ctx->aw, ARM64_REG_##reg, ARM64_REG_X20, G_STRUCT_OFFSET (CateyesAgentContext, field))
...
#define EMIT_ARM64_STORE(field, reg) \
    gum_arm64_writer_put_str_reg_reg_offset (&ctx->aw, ARM64_REG_##reg, ARM64_REG_X20, G_STRUCT_OFFSET (CateyesAgentContext, field))
...
static void
cateyes_agent_context_emit_arm64_mach_stub_body (CateyesAgentContext * self, CateyesAgentEmitContext * ctx)
{
  const gchar * again = "again";
...
static void
cateyes_agent_context_emit_arm64_pthread_stub_body (CateyesAgentContext * self, CateyesAgentEmitContext * ctx)
{
  const gchar * skip_construction = "skip_construction";
  const gchar * skip_dlopen = "skip_dlopen";
  const gchar * skip_destruction = "skip_destruction";
  ...
  gum_arm64_writer_put_ldr_reg_u64 (&ctx->aw, ARM64_REG_X1, CATEYES_UNLOAD_POLICY_IMMEDIATE);
  ...
  EMIT_ARM64_LOAD (W0, unload_policy);
  gum_arm64_writer_put_ldr_reg_u64 (&ctx->aw, ARM64_REG_X1, CATEYES_UNLOAD_POLICY_DEFERRED);
  gum_arm64_writer_put_cmp_reg_reg (&ctx->aw, ARM64_REG_W0, ARM64_REG_W1);
...
cateyes_get_debug_state (mach_port_t thread, gpointer state, GumCpuType cpu_type)
{
...
static kern_return_t
cateyes_set_debug_state (mach_port_t thread, gconstpointer state, GumCpuType cpu_type)
{
  mach_msg_type_number_t state_count;
  kern_return_t kr;
...
static void
cateyes_set_nth_hardware_breakpoint (gpointer state, guint n, GumAddress break_at, GumCpuType cpu_type)
{
#ifdef HAVE_I386
...
#else
# define CATEYES_S_USER ((uint32_t) (2u << 1))
# define CATEYES_BAS_ANY ((uint32_t) 15u)
# define CATEYES_BCR_ENABLE ((uint32_t) 1u)
  ...
  if (cpu_type == GUM_CPU_ARM64)
  {
    ...
    s->__bvr[n] = break_at;
    s->__bcr[n] = (CATEYES_BAS_ANY << 5) | CATEYES_S_USER | CATEYES_BCR_ENABLE;
  }
  else
  {
...
static GumAddress
cateyes_find_run_initializers_call (mach_port_t task, GumCpuType cpu_type, GumAddress start)
{
  GumAddress found = 0;
  ...
  capstone = cateyes_create_capstone (cpu_type, start);
...
static GumAddress
cateyes_find_function_end (mach_port_t task, GumCpuType cpu_type, GumAddress start, gsize max_size)
{
  GumAddress found = 0;
  uint64_t address = start & ~G_GUINT64_CONSTANT (1);
  csh capstone;
...
static csh
cateyes_create_capstone (GumCpuType cpu_type, GumAddress start)
{
  csh capstone;
  cs_err err;
...
static void
cateyes_mapper_library_blob_deallocate (CateyesMappedLibraryBlob * self)
{
  mach_vm_deallocate (mach_task_self (), self->_address, self->_allocated_size);

  cateyes_mapped_library_blob_free (self);
}
