namespace Cateyes.AgentTest {
	...
	namespace Script {
		private static async void load_and_receive_messages (Harness h) {
			...
			func (1337, "Cateyes rocks");

			var message = yield h.wait_for_message ();
			assert (message.sender_id.handle == sid.handle);
			assert (message.content == "{\"type\":\"send\",\"payload\":{\"first_argument\":1337,\"second_argument\":\"Cateyes rocks\"}}");
...
""");
				...
				for (uint i = 0; i != 1000000; i++) {
					...
					while (true) {
						...
						assert (reader.get_string_value () ==  "cateyes:rpc");
						reader.end_element ();
					...
					var child = Cateyes.Test.Process.start ("/bin/ls", new string[] { "UIKitApplication:foo.bar.Baz[0x1234]" });
	...
	private class Harness : Cateyes.Test.AsyncHarness, AgentController {
		private GLib.Module module;
		[CCode (has_target = false)]
		private delegate void AgentMainFunc (string data, ref Cateyes.UnloadPolicy unload_policy, void * opaque_injector_state);
		...
		public async AgentSession load_agent () {
...
#if WINDOWS
			var intermediate_root_dir = Path.get_dirname (Path.get_dirname (Cateyes.Test.Process.current.filename));
			if (sizeof (void *) == 4)
				agent_filename = Path.build_filename (intermediate_root_dir, "cateyes-agent-32", "cateyes-agent-32.dll");
			else
				agent_filename = Path.build_filename (intermediate_root_dir, "cateyes-agent-64", "cateyes-agent-64.dll");
#else
...
#endif
#if IOS || ANDROID
			var deployment_dir = Path.get_dirname (Cateyes.Test.Process.current.filename);
			agent_filename = Path.build_filename (deployment_dir, "cateyes-agent." + shlib_extension);
#else
			var cateyes_root_dir = Path.get_dirname (Path.get_dirname (Cateyes.Test.Process.current.filename));
			agent_filename = Path.build_filename (cateyes_root_dir, "lib", "cateyes", "cateyes-agent." + shlib_extension);
			if (!FileUtils.test (agent_filename, FileTest.EXISTS))
				agent_filename = Path.build_filename (cateyes_root_dir, "lib", "agent", "cateyes-agent." + shlib_extension);
#endif
#endif
			...
			var main_func_found = module.symbol ("cateyes_agent_main", out main_func_symbol);
			...
			main_thread = new Thread<bool> ("cateyes-test-agent-worker", agent_main_worker);
		...
		private bool agent_main_worker () {
			Cateyes.UnloadPolicy unload_policy = IMMEDIATE;
