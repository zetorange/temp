namespace Cateyes.Agent {
	public void main (string pipe_address, ref Cateyes.UnloadPolicy unload_policy, void * injector_state) {
	...
	private class Runner : Object, AgentSessionProvider, ExitHandler, ForkHandler, SpawnHandler {
		...
		public static void create_and_run (string pipe_address, ref Cateyes.UnloadPolicy unload_policy, void * opaque_injector_state) {
			Environment._init ();
...
#if !WINDOWS
		public static void resume_after_fork (ref Cateyes.UnloadPolicy unload_policy, void * opaque_injector_state) {
			{
#if LINUX
				var injector_state = (LinuxInjectorState *) opaque_injector_state;
				if (injector_state != null)
	...
	private Gum.MemoryRange memory_range (Gum.MemoryRange? mapped_range) {
		...
		if (result == null) {
			Gum.Process.enumerate_modules ((details) => {
				if (details.name.index_of ("cateyes-agent") != -1) {
	...
	private static string _detect_libc_name () {
		...
		Gum.Process.enumerate_modules ((details) => {
			...
			if (details.path == "/usr/lib/libc-2.17.so") {
				libc_name = details.path;
				return false;
			}
			if (details.path == "/usr/lib64/libc-2.17.so") {
				libc_name = details.path;
				return false;
			}
