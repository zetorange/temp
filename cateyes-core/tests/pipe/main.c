...
{
  CateyesPipeTransport * transport = NULL;
  const gchar * address;
  CateyesPipe * pipe;
  gchar c;
  GError * error = NULL;
  ...
  if (argc == 1)
  {
    transport = cateyes_pipe_transport_new (NULL);
    address = cateyes_pipe_transport_get_local_address (transport);
    g_print ("listening on '%s'\n", cateyes_pipe_transport_get_remote_address (transport));
  }
  else
  {
    address = argv[1];
  ...
  pipe = cateyes_pipe_new (address, &error);
  if (error != NULL)
  {
    g_printerr ("cateyes_pipe_new failed: %s\n", error->message);
  }
  else
  {
    if (transport != NULL)
