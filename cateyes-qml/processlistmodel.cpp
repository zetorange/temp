...
void ProcessListModel::enumerateProcesses()
{
    ...
    cateyes_device_enumerate_processes(m_activeDevice->handle(), onEnumerateReadyWrapper, request);
}
...
void ProcessListModel::onEnumerateReady(GAsyncResult *res)
{
    ...
    auto processHandles = cateyes_device_enumerate_processes_finish(m_activeDevice->handle(), res, &error);
    if (error == nullptr) {
        QSet<unsigned int> current;
        QList<Process *> added;
        QSet<unsigned int> removed;

        const int size = cateyes_process_list_size(processHandles);
        for (int i = 0; i != size; i++) {
            auto processHandle = cateyes_process_list_get(processHandles, i);
            auto pid = cateyes_process_get_pid(processHandle);
            current.insert(pid);
