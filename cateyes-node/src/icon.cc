...
Icon::Icon(CateyesIcon* handle, Runtime* runtime)
...
NAN_METHOD(Icon::New) {
  ...
  auto handle = static_cast<CateyesIcon*>(
...
NAN_PROPERTY_GETTER(Icon::GetWidth) {
  auto handle = ObjectWrap::Unwrap<Icon>(
      info.Holder())->GetHandle<CateyesIcon>();

  info.GetReturnValue().Set(Nan::New<v8::Integer>(
    cateyes_icon_get_width(handle)));
}
...
NAN_PROPERTY_GETTER(Icon::GetHeight) {
  ...
  info.GetReturnValue().Set(Nan::New<v8::Integer>(
    cateyes_icon_get_height(handle)));
}
...
NAN_PROPERTY_GETTER(Icon::GetRowstride) {
  ...
  info.GetReturnValue().Set(Nan::New<v8::Integer>(
    cateyes_icon_get_rowstride(handle)));
}
...
NAN_PROPERTY_GETTER(Icon::GetPixels) {
  ...
  auto bytes = cateyes_icon_get_pixels(handle);
  gsize size;
