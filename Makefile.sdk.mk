...
repo_base_url := "git://github.com/cateyes"
repo_suffix := ".git"
...
ifdef CATEYES_HOST
	host_platform := $(shell echo $(CATEYES_HOST) | cut -f1 -d"-")
else
	host_platform := $(build_platform)
endif
ifdef CATEYES_HOST
	host_arch := $(shell echo $(CATEYES_HOST) | cut -f2 -d"-")
else
...
all: build/sdk-$(host_platform)-$(host_arch).tar.bz2
	...
	@echo ""
	@echo "It will be picked up automatically if you now proceed to build Cateyes."
	@echo ""
...
ifeq ($(host_platform), ios)
	cp $(shell xcrun --sdk macosx --show-sdk-path)/usr/include/mach/mach_vm.h $(@D)/package/include/cateyes_mach_vm.h
endif
	@touch $@
...
build/fs-tmp-%/libiconv/Makefile: build/fs-env-%.rc build/.libiconv-stamp
	...
	. $< \
		&& cd $(@D) \
		&& CATEYES_LEGACY_AUTOTOOLS=1 ../../../libiconv/configure \
			--enable-static \
			--disable-shared \
			--enable-relocatable \
			--disable-rpath
...
build/fs-tmp-%/elfutils/Makefile: build/fs-env-%.rc build/.elfutils-stamp build/fs-%/lib/pkgconfig/liblzma.pc
	...
	. $< \
		&& cd $(@D) \
		&& if [ -n "$$CATEYES_GCC" ]; then \
			export CC="$$CATEYES_GCC"; \
		fi \
		&& ../../../elfutils/configure
...
build/fs-tmp-%/.v8-source-stamp: build/.v8-stamp
	...
	@mkdir -p $(@D)
	$(RM) -r $(@D)/v8
	# git clone $(repo_base_url)/v8$(repo_suffix) $(@D)/v8
	cp -r v8 $(@D)/
	@touch $@
...
build/fs-%/lib/pkgconfig/v8.pc: build/fs-tmp-%/.v8-build-stamp
	...
	install -d $(@D)
	echo "prefix=\$${cateyes_sdk_prefix}" > $@.tmp
