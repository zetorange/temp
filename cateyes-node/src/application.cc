...
Application::Application(CateyesApplication* handle, Runtime* runtime)
...
NAN_METHOD(Application::New) {
  ...
  auto handle = static_cast<CateyesApplication*>(
...
NAN_PROPERTY_GETTER(Application::GetIdentifier) {
  auto handle = ObjectWrap::Unwrap<Application>(
      info.Holder())->GetHandle<CateyesApplication>();

  info.GetReturnValue().Set(
      Nan::New(cateyes_application_get_identifier(handle)).ToLocalChecked());
}
...
NAN_PROPERTY_GETTER(Application::GetName) {
  ...
  info.GetReturnValue().Set(
      Nan::New(cateyes_application_get_name(handle)).ToLocalChecked());
}
...
NAN_PROPERTY_GETTER(Application::GetPid) {
  ...
  info.GetReturnValue().Set(
      Integer::NewFromUnsigned(isolate, cateyes_application_get_pid(handle)));
}

NAN_PROPERTY_GETTER(Application::GetSmallIcon) {
  auto wrapper = ObjectWrap::Unwrap<Application>(info.Holder());
  auto handle = wrapper->GetHandle<CateyesApplication>();

  info.GetReturnValue().Set(
      Icon::New(cateyes_application_get_small_icon(handle), wrapper->runtime_));
}
...
NAN_PROPERTY_GETTER(Application::GetLargeIcon) {
  ...
  info.GetReturnValue().Set(
      Icon::New(cateyes_application_get_large_icon(handle), wrapper->runtime_));
}
