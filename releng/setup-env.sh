...
if [ -n "$CATEYES_HOST" ]; then
  host_platform=$(echo -n $CATEYES_HOST | cut -f1 -d"-")
else
  host_platform=$build_platform
fi
if [ -n "$CATEYES_HOST" ]; then
  host_arch=$(echo -n $CATEYES_HOST | cut -f2 -d"-")
else
...
case $CATEYES_ASAN in
  yes|no)
    enable_asan=$CATEYES_ASAN
    ;;
  *)
    enable_asan=no
    ;;
...
if [ -z "$CATEYES_HOST" ]; then
  echo "Assuming host is $host_platform_arch Set CATEYES_HOST to override."
fi
    ...
    case $ndk_installed_version in
      ...
      *)
        (
          ...
          echo ""
          echo "Cateyes's SDK - the prebuilt dependencies snapshot - was compiled against $ndk_required_name,"
          ...
          echo "  make -f Makefile.sdk.mk CATEYES_HOST=android-arm"
...
if [ -n "$CATEYES_ENV_NAME" ]; then
  cateyes_env_name_prefix=${CATEYES_ENV_NAME}-
else
  cateyes_env_name_prefix=
fi

pushd $releng_path/../ > /dev/null
CATEYES_ROOT=`pwd`
popd > /dev/null
CATEYES_BUILD="$CATEYES_ROOT/build"
CATEYES_PREFIX="$CATEYES_BUILD/${CATEYES_ENV_NAME:-cateyes}-${host_platform_arch}"
CATEYES_PREFIX_LIB="$CATEYES_PREFIX/lib"
CATEYES_TOOLROOT="$CATEYES_BUILD/${cateyes_env_name_prefix}toolchain-${build_platform_arch}"
CATEYES_SDKROOT="$CATEYES_BUILD/${cateyes_env_name_prefix}sdk-${host_platform_arch}"
...
case $host_platform in
  linux)
    ...
    CXX="$CATEYES_ROOT/releng/cxx-wrapper-linux.sh ${host_toolprefix}g++ -static-libgcc -static-libstdc++"
  ...
  macos)
    ...
    cc_wrapper=$CATEYES_BUILD/${CATEYES_ENV_NAME:-cateyes}-${host_platform_arch}-clang
    sed \
    ...
    cxx_wrapper=$CATEYES_BUILD/${CATEYES_ENV_NAME:-cateyes}-${host_platform_arch}-clang++
    sed \
    ...
    NM="$CATEYES_ROOT/releng/llvm-nm-macos-x86_64"
...
-no-canonical-prefixes"
    ...
    mkdir -p "$CATEYES_BUILD"

    cc_wrapper="$CATEYES_BUILD/${CATEYES_ENV_NAME:-cateyes}-cc-wrapper-${host_platform_arch}.sh"
    sed \
    ...
    cxx_wrapper="$CATEYES_BUILD/${CATEYES_ENV_NAME:-cateyes}-cxx-wrapper-${host_platform_arch}.sh"
    ...
    elf_cleaner=${CATEYES_ROOT}/releng/cateyes-elf-cleaner-${build_platform_arch}

    meson_cc_wrapper=$CATEYES_BUILD/${CATEYES_ENV_NAME:-cateyes}-${host_platform_arch}-clang
    sed \
    ...
    meson_cxx_wrapper=$CATEYES_BUILD/${CATEYES_ENV_NAME:-cateyes}-${host_platform_arch}-clang++
    sed \
      -e "s,@linker@,$android_clang_prefix/bin/clang++,g" \
      -e "s,@elf_cleaner@,$elf_cleaner,g" \
...
'-I$ANDROID_NDK_ROOT/sources/android/support/include'"
  ...
  qnx)
    ...
    qnx_preprocessor_flags="-include $CATEYES_ROOT/releng/cateyes-qnx-defines.h"
    ...
    CC="$CATEYES_ROOT/releng/cxx-wrapper-qnx.sh $qnx_toolchain_prefix-gcc $toolchain_flags -static-libgcc"
    CXX="$CATEYES_ROOT/releng/cxx-wrapper-qnx.sh $qnx_toolchain_prefix-g++ $toolchain_flags -static-libgcc -static-libstdc++"
...
legacy_includes="-I$CATEYES_PREFIX/include"
legacy_libpaths="-L$CATEYES_PREFIX/lib"
if [ "$CATEYES_ENV_SDK" != 'none' ]; then
  legacy_includes="$legacy_includes -I$CATEYES_SDKROOT/include"
  legacy_libpaths="$legacy_libpaths -L$CATEYES_SDKROOT/lib"
fi
...
if [ "$CATEYES_ENV_SDK" != 'none' ]; then
  version_include="-include $CATEYES_BUILD/cateyes-version.h"
  CPPFLAGS="$version_include $CPPFLAGS"
  CFLAGS="$version_include $CFLAGS"

  meson_version_include=", '-include', '$CATEYES_BUILD/cateyes-version.h'"
else
  meson_version_include=""
fi

ACLOCAL_FLAGS="-I $CATEYES_PREFIX/share/aclocal"
if [ "$CATEYES_ENV_SDK" != 'none' ]; then
  ACLOCAL_FLAGS="$ACLOCAL_FLAGS -I $CATEYES_SDKROOT/share/aclocal"
fi
ACLOCAL_FLAGS="$ACLOCAL_FLAGS -I $CATEYES_TOOLROOT/share/aclocal"
ACLOCAL="aclocal $ACLOCAL_FLAGS"
CONFIG_SITE="$CATEYES_BUILD/${cateyes_env_name_prefix}config-${host_platform_arch}.site"

VALAC="$CATEYES_TOOLROOT/bin/valac-0.42"
VALAFLAGS="--vapidir=\"$CATEYES_PREFIX/share/vala/vapi\""
if [ "$CATEYES_ENV_SDK" != 'none' ]; then
  VALAFLAGS="$VALAFLAGS --vapidir=\"$CATEYES_SDKROOT/share/vala/vapi\""
fi

[ ! -d "$CATEYES_PREFIX/share/aclocal}" ] && mkdir -p "$CATEYES_PREFIX/share/aclocal"
[ ! -d "$CATEYES_PREFIX/lib}" ] && mkdir -p "$CATEYES_PREFIX/lib"

if ! grep -Eq "^$toolchain_version\$" "$CATEYES_TOOLROOT/.version" 2>/dev/null; then
  rm -rf "$CATEYES_TOOLROOT"
  mkdir -p "$CATEYES_TOOLROOT"

  local_toolchain=toolchain-linux-x86_64.tar.bz2
  if [ -f $local_toolchain ]; then
    echo "Deploying local toolchain $local_toolchain..."
    tar -C "$CATEYES_TOOLROOT" -xjf $local_toolchain || exit 1
  else
    echo "Downloading and deploying toolchain..."
    echo "https://build.cateyes.re/toolchain-${toolchain_version}-${build_platform}-${build_arch}.tar.bz2"
    echo "$CATEYES_TOOLROOT"
    echo $local_toolchain
    $download_command "https://build.cateyes.re/toolchain-${toolchain_version}-${build_platform}-${build_arch}.tar.bz2" | tar -C "$CATEYES_TOOLROOT" -xj $tar_stdin || exit 1
  fi

  for template in $(find $CATEYES_TOOLROOT -name "*.cateyes.in"); do
    target=$(echo $template | sed 's,\.cateyes\.in$,,')
    cp -a "$template" "$target"
    sed \
      -e "s,@CATEYES_TOOLROOT@,$CATEYES_TOOLROOT,g" \
      "$template" > "$target"
  done

  vala_wrapper=$CATEYES_TOOLROOT/bin/valac-0.42
  vala_impl=$CATEYES_TOOLROOT/bin/valac-0.42-impl
  mv "$vala_wrapper" "$vala_impl"
  (
    echo "#!/bin/sh"
    echo "exec \"$vala_impl\" \"\$@\" --vapidir=\"$CATEYES_TOOLROOT/share/vala-0.42/vapi\""
  ) > "$vala_wrapper"
  chmod 755 "$vala_wrapper"

  ln -s "${CATEYES_ROOT}/releng/ninja-${build_platform_arch}" "$CATEYES_TOOLROOT/bin/ninja"
  ln -s "${CATEYES_ROOT}/releng/cateyes-resource-compiler-${build_platform_arch}" "$CATEYES_TOOLROOT/bin/cateyes-resource-compiler"

  echo $toolchain_version > "$CATEYES_TOOLROOT/.version"
fi

if [ "$CATEYES_ENV_SDK" != 'none' ] && ! grep -Eq "^$sdk_version\$" "$CATEYES_SDKROOT/.version" 2>/dev/null; then
  rm -rf "$CATEYES_SDKROOT"
  mkdir -p "$CATEYES_SDKROOT"

  local_sdk=$CATEYES_BUILD/sdk-${host_platform}-${host_arch}.tar.bz2
  if [ -f $local_sdk ]; then
    echo "Deploying local SDK $(basename $local_sdk)..."
    tar -C "$CATEYES_SDKROOT" -xjf $local_sdk || exit 1
  else
    echo "Downloading and deploying SDK for ${host_platform_arch}..."
    $download_command "https://build.cateyes.re/sdk-${sdk_version}-${host_platform}-${host_arch}.tar.bz2" | tar -C "$CATEYES_SDKROOT" -xj $tar_stdin 2> /dev/null
    if [ $? -ne 0 ]; then
      echo ""
  ...
  for template in $(find $CATEYES_SDKROOT -name "*.cateyes.in"); do
    target=$(echo $template | sed 's,\.cateyes\.in$,,')
    cp -a "$template" "$target"
    sed \
      -e "s,@CATEYES_SDKROOT@,$CATEYES_SDKROOT,g" \
      "$template" > "$target"
  done

  echo $sdk_version > "$CATEYES_SDKROOT/.version"
fi

strip_wrapper=$CATEYES_BUILD/${CATEYES_ENV_NAME:-cateyes}-${host_platform_arch}-strip
(
  echo "#!/bin/sh"
  echo "for arg in \"\$@\"; do"
...
PKG_CONFIG=$CATEYES_BUILD/${CATEYES_ENV_NAME:-cateyes}-${host_platform_arch}-pkg-config

pkg_config="$CATEYES_TOOLROOT/bin/pkg-config"
pkg_config_flags=""
pkg_config_path="$CATEYES_PREFIX_LIB/pkgconfig"
if [ "$CATEYES_ENV_SDK" != 'none' ]; then
  pkg_config_flags=" --define-variable=cateyes_sdk_prefix=$CATEYES_SDKROOT"
  pkg_config_path="$pkg_config_path:$CATEYES_SDKROOT/lib/pkgconfig"
fi
(
  echo "#!/bin/sh"
...
env_rc=build/${CATEYES_ENV_NAME:-cateyes}-env-${host_platform_arch}.rc
meson_env_rc=build/${CATEYES_ENV_NAME:-cateyes}-meson-env-${host_platform_arch}.rc

if [ "$CATEYES_ENV_SDK" != 'none' ]; then
  env_path_sdk="$CATEYES_SDKROOT/bin:"
else
  env_path_sdk=""
fi

(
  echo "export PATH=\"${env_path_sdk}${CATEYES_TOOLROOT}/bin:\$PATH\""
  echo "export PS1=\"\e[0;${prompt_color}m[\u@\h \w \e[m\e[1;${prompt_color}mcateyes-${host_platform_arch}\e[m\e[0;${prompt_color}m]\e[m\n\$ \""
...
sed \
  -e "s,@cateyes_host_platform@,$host_platform,g" \
  -e "s,@cateyes_host_arch@,$host_arch,g" \
  -e "s,@cateyes_host_platform_arch@,$host_platform_arch,g" \
  -e "s,@cateyes_prefix@,$CATEYES_PREFIX,g" \
  -e "s,@cateyes_optimization_flags@,$CATEYES_OPTIMIZATION_FLAGS,g" \
  -e "s,@cateyes_debug_flags@,$CATEYES_DEBUG_FLAGS,g" \
  $releng_path/config.site.in > "$CONFIG_SITE"

meson_cross_file=build/${CATEYES_ENV_NAME:-cateyes}-${host_platform_arch}.txt
...
echo "# source ${CATEYES_ROOT}/$env_rc"
