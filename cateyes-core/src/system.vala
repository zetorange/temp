namespace Cateyes {
	namespace System {
		public extern static Cateyes.HostApplicationInfo get_frontmost_application () throws Error;
		public extern static Cateyes.HostApplicationInfo[] enumerate_applications ();
		public extern static Cateyes.HostProcessInfo[] enumerate_processes ();
	...
	public class ApplicationEnumerator {
		...
		public async HostApplicationInfo[] enumerate_applications () {
			...
			if (is_first_request) {
				...
				new Thread<bool> ("cateyes-enumerate-applications", enumerate_applications_worker);
			}
	...
	public class ProcessEnumerator {
		...
		public async HostProcessInfo[] enumerate_processes () {
			...
			if (is_first_request) {
				current_main_context = MainContext.get_thread_default ();
				new Thread<bool> ("cateyes-enumerate-processes", enumerate_processes_worker);
			}
			pending_requests.add (request);
			yield;
	...
	public class TemporaryDirectory {
		...
		public static string make_name () {
			var builder = new StringBuilder ("cateyes-");
			for (var i = 0; i != 16; i++)
