...
DeviceManager::DeviceManager(CateyesDeviceManager* handle, Runtime* runtime)
...
void DeviceManager::Dispose(Runtime* runtime) {
  ...
  while (wrappers != NULL) {
    ...
    cateyes_device_manager_close_sync(wrapper->GetHandle<CateyesDeviceManager>());
...
NAN_METHOD(DeviceManager::New) {
  ...
  auto handle = cateyes_device_manager_new();
...
class CloseOperation : public Operation<CateyesDeviceManager> {
 public:
  void Begin() {
    cateyes_device_manager_close(handle_, OnReady, this);
  }

  void End(GAsyncResult* result, GError** error) {
    cateyes_device_manager_close_finish(handle_, result);
  }
...
class EnumerateDevicesOperation : public Operation<CateyesDeviceManager> {
 public:
  void Begin() {
    cateyes_device_manager_enumerate_devices(handle_, OnReady, this);
  }

  void End(GAsyncResult* result, GError** error) {
    devices_ = cateyes_device_manager_enumerate_devices_finish(handle_, result,
        error);
  }

  Local<Value> Result(Isolate* isolate) {
    auto size = cateyes_device_list_size(devices_);
    Local<v8::Array> devices = Nan::New<v8::Array>(size);
    for (auto i = 0; i != size; i++) {
      auto handle = cateyes_device_list_get(devices_, i);
      auto device = Device::New(handle, runtime_);
      Nan::Set(devices, i, device);
      g_object_unref(handle);
    }

    cateyes_unref(devices_);

    return devices;
  }

  CateyesDeviceList* devices_;
};
...
class AddRemoteDeviceOperation : public Operation<CateyesDeviceManager> {
 public:
  ...
  void Begin() {
    cateyes_device_manager_add_remote_device(handle_, host_, OnReady, this);
  }

  void End(GAsyncResult* result, GError** error) {
    device_ = cateyes_device_manager_add_remote_device_finish(handle_, result,
        error);
  }
...
class RemoveRemoteDeviceOperation : public Operation<CateyesDeviceManager> {
 public:
  ...
  void Begin() {
    cateyes_device_manager_remove_remote_device(handle_, host_, OnReady, this);
  }

  void End(GAsyncResult* result, GError** error) {
    cateyes_device_manager_remove_remote_device_finish(handle_, result, error);
  }
