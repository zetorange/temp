...
public class Device: NSObject, NSCopying {
    ...
    public var id: String {
        return String(cString: cateyes_device_get_id(handle))
    }

    public var name: String {
        return String(cString: cateyes_device_get_name(handle))
    }

    public var icon: NSImage? {
        return Marshal.imageFromIcon(cateyes_device_get_icon(handle))
    }
    ...
    public var kind: Kind {
        ...
        case CATEYES_DEVICE_TYPE_USB:
            return Kind.usb
        default:
            fatalError("Unexpected Cateyes Device kind")
        }
    }

    public override var description: String {
        return "Cateyes.Device(id: \"\(id)\", name: \"\(name)\", kind: \"\(kind)\")"
    }
    ...
    public func getFrontmostApplication(_ completionHandler: @escaping GetFrontmostApplicationComplete) {
        Runtime.scheduleOnCateyesThread {
            cateyes_device_get_frontmost_application(self.handle, { source, result, data in
                let operation = Unmanaged<AsyncOperation<GetFrontmostApplicationComplete>>.fromOpaque(data!).takeRetainedValue()

                var rawError: UnsafeMutablePointer<GError>? = nil
                let rawApplication = cateyes_device_get_frontmost_application_finish(OpaquePointer(source), result, &rawError)
    ...
    public func enumerateApplications(_ completionHandler: @escaping EnumerateApplicationsComplete) {
        Runtime.scheduleOnCateyesThread {
            cateyes_device_enumerate_applications(self.handle, { source, result, data in
                let operation = Unmanaged<AsyncOperation<EnumerateApplicationsComplete>>.fromOpaque(data!).takeRetainedValue()

                var rawError: UnsafeMutablePointer<GError>? = nil
                let rawApplications = cateyes_device_enumerate_applications_finish(OpaquePointer(source), result, &rawError)
                if let rawError = rawError {
                    let error = Marshal.takeNativeError(rawError)
                    Runtime.scheduleOnMainThread {
                        operation.completionHandler { throw error }
                ...
                let numberOfApplications = cateyes_application_list_size(rawApplications)
                for index in 0..<numberOfApplications {
                    let application = ApplicationDetails(handle: cateyes_application_list_get(rawApplications, index))
    ...
    public func enumerateProcesses(_ completionHandler: @escaping EnumerateProcessesComplete) {
        Runtime.scheduleOnCateyesThread {
            cateyes_device_enumerate_processes(self.handle, { source, result, data in
                let operation = Unmanaged<AsyncOperation<EnumerateProcessesComplete>>.fromOpaque(data!).takeRetainedValue()

                var rawError: UnsafeMutablePointer<GError>? = nil
                let rawProcesses = cateyes_device_enumerate_processes_finish(OpaquePointer(source), result, &rawError)
                if let rawError = rawError {
                    let error = Marshal.takeNativeError(rawError)
                    Runtime.scheduleOnMainThread {
                        operation.completionHandler { throw error }
                ...
                let numberOfProcesses = cateyes_process_list_size(rawProcesses)
                for index in 0..<numberOfProcesses {
                    let process = ProcessDetails(handle: cateyes_process_list_get(rawProcesses, index))
    ...
    public func enableSpawnGating(_ completionHandler: @escaping EnableSpawnGatingComplete = { _ in }) {
        Runtime.scheduleOnCateyesThread {
            cateyes_device_enable_spawn_gating(self.handle, { source, result, data in
                let operation = Unmanaged<AsyncOperation<EnableSpawnGatingComplete>>.fromOpaque(data!).takeRetainedValue()

                var rawError: UnsafeMutablePointer<GError>? = nil
                cateyes_device_enable_spawn_gating_finish(OpaquePointer(source), result, &rawError)
                if let rawError = rawError {
                    let error = Marshal.takeNativeError(rawError)
                    Runtime.scheduleOnMainThread {
                        operation.completionHandler { throw error }
    ...
    public func disableSpawnGating(_ completionHandler: @escaping DisableSpawnGatingComplete = { _ in }) {
        Runtime.scheduleOnCateyesThread {
            cateyes_device_disable_spawn_gating(self.handle, { source, result, data in
                let operation = Unmanaged<AsyncOperation<DisableSpawnGatingComplete>>.fromOpaque(data!).takeRetainedValue()

                var rawError: UnsafeMutablePointer<GError>? = nil
                cateyes_device_disable_spawn_gating_finish(OpaquePointer(source), result, &rawError)
                if let rawError = rawError {
                    let error = Marshal.takeNativeError(rawError)
                    Runtime.scheduleOnMainThread {
                        operation.completionHandler { throw error }
    ...
    public func enumeratePendingSpawn(_ completionHandler: @escaping EnumeratePendingSpawnComplete) {
        Runtime.scheduleOnCateyesThread {
            cateyes_device_enumerate_pending_spawn(self.handle, { source, result, data in
                let operation = Unmanaged<AsyncOperation<EnumeratePendingSpawnComplete>>.fromOpaque(data!).takeRetainedValue()

                var rawError: UnsafeMutablePointer<GError>? = nil
                let rawSpawn = cateyes_device_enumerate_pending_spawn_finish(OpaquePointer(source), result, &rawError)
                if let rawError = rawError {
                    let error = Marshal.takeNativeError(rawError)
                    Runtime.scheduleOnMainThread {
                        operation.completionHandler { throw error }
                ...
                let numberOfSpawn = cateyes_spawn_list_size(rawSpawn)
                for index in 0..<numberOfSpawn {
                    let details = SpawnDetails(handle: cateyes_spawn_list_get(rawSpawn, index))
    ...
    public func spawn(_ program: String, argv: [String]? = nil, envp: [String: String]? = nil, env: [String: String]? = nil,
        ...
        Runtime.scheduleOnCateyesThread {
            ...
            if let stdio = stdio {
                cateyes_spawn_options_set_stdio(options, CateyesStdio(UInt32(stdio.rawValue)))
            }

            cateyes_device_spawn(self.handle, program, options, { source, result, data in
                let operation = Unmanaged<AsyncOperation<SpawnComplete>>.fromOpaque(data!).takeRetainedValue()

                var rawError: UnsafeMutablePointer<GError>? = nil
                let pid = cateyes_device_spawn_finish(OpaquePointer(source), result, &rawError)
                if let rawError = rawError {
                    let error = Marshal.takeNativeError(rawError)
                    Runtime.scheduleOnMainThread {
                        operation.completionHandler { throw error }
    ...
    public func input(_ pid: UInt, data: Data, completionHandler: @escaping InputComplete = { _ in }) {
        Runtime.scheduleOnCateyesThread {
            ...
            cateyes_device_input(self.handle, guint(pid), rawData, { source, result, data in
                let operation = Unmanaged<AsyncOperation<InputComplete>>.fromOpaque(data!).takeRetainedValue()

                var rawError: UnsafeMutablePointer<GError>? = nil
                cateyes_device_input_finish(OpaquePointer(source), result, &rawError)
                if let rawError = rawError {
                    let error = Marshal.takeNativeError(rawError)
                    Runtime.scheduleOnMainThread {
                        operation.completionHandler { throw error }
    ...
    public func resume(_ pid: UInt, completionHandler: @escaping ResumeComplete = { _ in }) {
        Runtime.scheduleOnCateyesThread {
            cateyes_device_resume(self.handle, guint(pid), { source, result, data in
                let operation = Unmanaged<AsyncOperation<ResumeComplete>>.fromOpaque(data!).takeRetainedValue()

                var rawError: UnsafeMutablePointer<GError>? = nil
                cateyes_device_resume_finish(OpaquePointer(source), result, &rawError)
                if let rawError = rawError {
                    let error = Marshal.takeNativeError(rawError)
                    Runtime.scheduleOnMainThread {
                        operation.completionHandler { throw error }
    ...
    public func kill(_ pid: UInt, completionHandler: @escaping KillComplete = { _ in }) {
        Runtime.scheduleOnCateyesThread {
            cateyes_device_kill(self.handle, guint(pid), { source, result, data in
                let operation = Unmanaged<AsyncOperation<KillComplete>>.fromOpaque(data!).takeRetainedValue()

                var rawError: UnsafeMutablePointer<GError>? = nil
                cateyes_device_kill_finish(OpaquePointer(source), result, &rawError)
                if let rawError = rawError {
                    let error = Marshal.takeNativeError(rawError)
                    Runtime.scheduleOnMainThread {
                        operation.completionHandler { throw error }
    ...
    public func attach(_ pid: UInt, completionHandler: @escaping AttachComplete) {
        Runtime.scheduleOnCateyesThread {
            cateyes_device_attach(self.handle, guint(pid), { source, result, data in
                let operation = Unmanaged<AsyncOperation<AttachComplete>>.fromOpaque(data!).takeRetainedValue()

                var rawError: UnsafeMutablePointer<GError>? = nil
                let rawSession = cateyes_device_attach_finish(OpaquePointer(source), result, &rawError)
                if let rawError = rawError {
                    let error = Marshal.takeNativeError(rawError)
                    Runtime.scheduleOnMainThread {
                        operation.completionHandler { throw error }
