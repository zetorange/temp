...
namespace Winjector {
	...
	public class Manager : Object, WinjectorHelper {
		...
		public async void stop () throws Cateyes.Error {
			if (System.is_x64 ()) {
				try {
		...
		public async uint inject_library_file (uint pid, string path_template, string entrypoint, string data) throws Cateyes.Error {
			try {
		...
		private class HelperService {
			...
			public async void start () throws Cateyes.Error {
				try {
					...
					throw new Cateyes.Error.PERMISSION_DENIED (e.message);
				}

				try {
					proxy = yield connection.get_proxy (null, WinjectorObjectPath.HELPER);
				} catch (IOError e) {
					throw new Cateyes.Error.PROTOCOL (e.message);
				}
			}
		}
	...
	public abstract class Service : Object, WinjectorHelper {
		...
		private async void do_stop () throws Cateyes.Error {
			connection.unregister_object (registration_id);
			connection.on_closed.disconnect (on_connection_closed);
			try {
				yield connection.close ();
		...
		public async uint inject_library_file (uint pid, string path, string entrypoint, string data) throws Cateyes.Error {
	...
	namespace Process {
		...
		public extern static void inject_library_file (uint32 pid, string path, string entrypoint, string data, out void * inject_instance, out void * waitable_thread_handle) throws Cateyes.Error;
